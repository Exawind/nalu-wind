...
main(int argc, char** argv)
{
...

  {
    ...
    const std::string kynema-ugfVersion = (version::RepoIsDirty == "DIRTY")
                                      ? (version::KynemaUGFVersionTag + "-dirty")
                                      : version::KynemaUGFVersionTag;

    stk::OptionsSpecification desc("KynemaUGF Supported Options");
    std::string kynema-ugfVout = kynema-ugfVersion.c_str();
    desc.add_options()("help,h", "Help message")(
      "version,v", kynema-ugfVersion.c_str())(
      "input-deck,i", "Analysis input file",
      stk::DefaultValue<std::string>("kynema-ugf.i"),
    ...
    if (parsedOptions.count("version")) {
      if (!kynema-ugfEnv.parallel_rank())
        std::cerr << "Version: " << kynema-ugfVersion << std::endl;
      return 0;
    }
    ...
    kynema-ugfEnv.set_log_file_stream(logFileName, pprint, capture_stdout);
    ...
    // stop timer
    const double stop_time = kynema-ugfEnv.kynema-ugf_time();
    ...
    stk::all_reduce_min(kynema-ugfEnv.parallel_comm(), &total_time, &g_min, 1);
    stk::all_reduce_max(kynema-ugfEnv.parallel_comm(), &total_time, &g_max, 1);
    stk::all_reduce_sum(kynema-ugfEnv.parallel_comm(), &total_time, &g_sum, 1);
    const int nprocs = kynema-ugfEnv.parallel_size();

    // output total time
    kynema-ugfEnv.kynema-ugfOutputP0() << "Timing for Simulation: nprocs= " << nprocs
                           << std::endl;
    kynema-ugfEnv.kynema-ugfOutputP0() << "           main() --  "
    ...
    // output memory usage
    {
      ...
      stk::all_reduce(
        kynema-ugfEnv.parallel_comm(), stk::ReduceSum<1>(&global_now[2]));
      stk::all_reduce(
        kynema-ugfEnv.parallel_comm(), stk::ReduceMin<1>(&global_now[0]));
      stk::all_reduce(
        kynema-ugfEnv.parallel_comm(), stk::ReduceMax<1>(&global_now[1]));

      stk::all_reduce(
        kynema-ugfEnv.parallel_comm(), stk::ReduceSum<1>(&global_hwm[2]));
      stk::all_reduce(
        kynema-ugfEnv.parallel_comm(), stk::ReduceMin<1>(&global_hwm[0]));
      stk::all_reduce(
        kynema-ugfEnv.parallel_comm(), stk::ReduceMax<1>(&global_hwm[1]));

      kynema-ugfEnv.kynema-ugfOutputP0() << "Memory Overview: " << std::endl;
    ...
    stk::parallel_print_time_without_output_and_hwm(
      kynema-ugfEnv.parallel_comm(), time_without_output, kynema-ugfEnv.kynema-ugfOutputP0());
