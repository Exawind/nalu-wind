...
inline std::string
vector_string(double x, double y)
{
  std::stringstream ss;
  ss << "(" << clipv(x) << ", " << clipv(y) << ")";
  return ss.str();
...
template <typename T>
void
dump_2d_vector_view(const T& v)
{
  for (unsigned j = 0; j < v.extent(1); ++j) {
    for (unsigned i = 0; i < v.extent(0); ++i) {
      std::cout << std::setw(5) << v.label() << "(" << i << ", " << j
                << "): " << std::setw(5)
                << vector_string(v(0, j, i), v(1, j, i));
      if (i != v.extent(0) - 1) {
        std::cout << ", ";
      }
    }
    ...
    for (unsigned k = 0; k < x.extent(0); ++k) {                               \
      for (unsigned j = 0; j < x.extent(1); ++j) {                             \
        for (unsigned i = 0; i < x.extent(2); ++i) {                           \
          ASSERT_NEAR(x(k, j, i), y(k, j, i), tol)                             \
            << "(k,j,i) = (" << k << ", " << j << ", " << i << ")";            \
        }                                                                      \
      }                                                                        \
    }                                                                          \
...
#define TEST_POLY(x, y, z)                                                     \
  TEST(x, y##_##order_##z) { y<z>(); }

#define TEST_POLY_to5(x, y)                                                    \
  TEST(x, y##_##order_##1to7)                                                  \
  {                                                                            \
    y<1>();                                                                    \
    y<2>();                                                                    \
    y<3>();                                                                    \
