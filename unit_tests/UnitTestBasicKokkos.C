...
TEST(BasicKokkos, discover_execution_space)
{
  ...
  if (proc == 0) {
    ...
    std::cout << "Kokkos::OpenMP is available. (Control num-threads via "
...
run_parallel_for_test()
{
  ...
  // Important note: when the 'host' and 'device' share the same memory space,
...
TEST(BasicKokkos, parallel_for) { run_parallel_for_test(); }
...
run_nested_parallel_for_thread_teams_test()
{
  ...
  typedef typename Kokkos::TeamPolicy<
    typename Kokkos::DefaultExecutionSpace, DynamicScheduleType>::member_type
    TeamHandleType;
  ...
  Kokkos::parallel_for(
    ...
    KOKKOS_LAMBDA(const TeamHandleType& team) {
      ...
      Kokkos::parallel_for(
        Kokkos::TeamThreadRange(team, (size_t)0, M),
        [&](const size_t& j) { device_view2D(i, j) *= 2; });
    });
