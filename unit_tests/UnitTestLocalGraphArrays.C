...
TEST(LocalGraphArrays, compute_row_pointers)
{
  unsigned N = 5;
  size_t nnz = 0;
  Kokkos::View<size_t*, sierra::kynema-ugf::HostSpace> rowLengths("rowLengths", N);
  for (unsigned i = 0; i < N; ++i) {
    rowLengths(i) = i + 2;
    nnz += rowLengths(i);
  }

  Kokkos::View<size_t*, sierra::kynema-ugf::HostSpace> rowPointers("rowPtrs", N + 1);
  sierra::kynema-ugf::LocalGraphArrays::compute_row_pointers(rowPointers, rowLengths);
  EXPECT_EQ(nnz, rowPointers(N));
}

Teuchos::RCP<sierra::kynema-ugf::LocalGraphArrays>
create_graph(const std::vector<size_t>& rowLens)
{
  ...
  return Teuchos::rcp(new sierra::kynema-ugf::LocalGraphArrays(rowLengths));
}

TEST(LocalGraphArrays, construct)
{
  Teuchos::RCP<sierra::kynema-ugf::LocalGraphArrays> csg = create_graph({2, 3, 4});
...
TEST(LocalGraphArrays, insertIndicesNumDof1)
{
  ...
  // now insert row-0 and row-1 cols again, then make sure the graph is still
  // correct.
...
{
  Teuchos::RCP<sierra::kynema-ugf::LocalGraphArrays> csg = create_graph({6, 9, 12});
