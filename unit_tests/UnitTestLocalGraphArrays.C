...
TEST(LocalGraphArrays, compute_row_pointers)
{
  unsigned N = 5;
  size_t nnz = 0;
  Kokkos::View<size_t*, sierra::kynema_ugf::HostSpace> rowLengths("rowLengths", N);
  for (unsigned i = 0; i < N; ++i) {
    rowLengths(i) = i + 2;
    nnz += rowLengths(i);
  }

  Kokkos::View<size_t*, sierra::kynema_ugf::HostSpace> rowPointers("rowPtrs", N + 1);
  sierra::kynema_ugf::LocalGraphArrays::compute_row_pointers(rowPointers, rowLengths);
...
{
  Teuchos::RCP<sierra::kynema_ugf::LocalGraphArrays> csg = create_graph({2, 3, 4});
...
{
  Teuchos::RCP<sierra::kynema_ugf::LocalGraphArrays> csg = create_graph({6, 9, 12});
