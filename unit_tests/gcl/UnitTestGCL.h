...
public:
        ...
        (stk::topology::NODE_RANK, "dvol_dt"))
  {
    ...
    stk::mesh::put_field_on_mesh(
      *sweptVol_, meta_.universal_part(), sierra::nalu::AlgTraitsHex8::numScsIp_,
      nullptr);
    stk::mesh::put_field_on_mesh(
      *faceVelMag_, meta_.universal_part(), sierra::nalu::AlgTraitsHex8::numScsIp_,
      nullptr);
    stk::mesh::put_field_on_mesh(
      *edgeSweptVol_, meta_.universal_part(), 1,
      nullptr);
    stk::mesh::put_field_on_mesh(
      *edgeFaceVelMag_, meta_.universal_part(), 1,
      nullptr);
    stk::mesh::put_field_on_mesh(
  ...
  void init_time_integrator(bool secondOrder=true, double timeStep=0.1, int timeStepCount=2)
  {
  ...
  void register_algorithms(const std::string& motion_options)
  {
    ...
    if (realm_.realmUsesEdges_) {
      geomAlgDriver_.register_elem_algorithm<
  ...
  {
    if (realm_.realmUsesEdges_) {
      sierra::nalu::compute_edge_scalar_divergence(bulk_, partVec_,
                                                   bndyPartVec_,
                                                   edgeFaceVelMag_,
                                                   divMeshVel_);      
    } else {
      sierra::nalu::compute_scalar_divergence(bulk_, partVec_,
                                              bndyPartVec_,
                                              faceVelMag_,
                                              divMeshVel_);
    }
  ...
  {
    return (
      meta_.universal_part() & !stk::mesh::selectUnion(bndyPartVec_));
  }
