...
namespace {
void
check_elem_to_side_coords(stk::topology topo)
{
  ...
  for (const auto* ib : face_buckets) {
    ...
    for (size_t k = 0; k < b.size(); ++k) {
      ...
      meSCS->sidePcoords_to_elemPcoords(
        side_ordinal, 1, sideIsoPoint.data(), sideElemCoords.data());

      std::vector<double> faceInterpCoords(dim, 0.0);
      meSide->interpolatePoint(
        dim, sideIsoPoint.data(), faceCoords.data(), faceInterpCoords.data());

      std::vector<double> isoFacePoint(dim, 0.0);
      meSCS->isInElement(
        elemCoords.data(), faceInterpCoords.data(), isoFacePoint.data());

      // quad/hex isInElement uses a different isoparameteric range
      double isoFac =
        (topo == stk::topology::HEX_8 || topo == stk::topology::QUAD_4_2D)
          ? 0.5
          : 1.0;
      for (int d = 0; d < dim; ++d) {
