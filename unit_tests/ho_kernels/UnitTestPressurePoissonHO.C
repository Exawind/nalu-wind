...
struct TestFunction
{
  ...
  {
    return -2 * (kx + ky) * std::cos(kz * z) * std::cos(2 * kz * z) *
             std::pow(std::sin(kx * x), 2) * std::pow(std::sin(ky * y), 2) +
           (ky * std::pow(std::cos(ky * y), 2) * std::pow(std::sin(kx * x), 2) +
            kx * std::pow(std::cos(kx * x), 2) *
              std::pow(std::sin(ky * y), 2)) *
             std::sin(kz * z) * std::sin(2 * kz * z);
  }
...
mms()
{
  ...
  nodal_scalar_workview<p, double> work_density;
  auto& density = work_density.view();
  ...
  nodal_scalar_workview<p, double> work_exact_resid;
  ...
  high_order_metrics::compute_laplacian_metric_linear(
    ops, coords, work_metric.view());

  scs_scalar_workview<p, double> work_mdot(0);
  high_order_metrics::compute_mdot_linear(
    ops, coords, work_metric.view(), 1.0, density, velocity, Gp, pressure,
    work_mdot.view());
  ...
  for (int k = 1; k < p; ++k) {
    for (int j = 1; j < p; ++j) {
      for (int i = 1; i < p; ++i) {
        EXPECT_NEAR(rhs(k, j, i), 0, 1.0e-8)
          << "(k,j,i) = (" << k << ", " << j << ", " << i << ")";
      }
    }
  }
