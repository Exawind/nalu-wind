...
struct TestFunction
{
  ...
  {
    return -(std::cos(2 * kx * x) *
             (2 * std::cos(ky * y) *
                (-2 *
                   (4 * std::pow(kx, 2) + std::pow(ky, 2) + std::pow(kz, 2)) *
                   std::cos(kz * z) +
                 std::pow(std::cos(kz * z), 2) *
                   (std::cos(kx * x) *
                      (2 * std::pow(ky, 2) -
                       (4 * std::pow(kx, 2) + 3 * std::pow(ky, 2) +
                        std::pow(kz, 2)) *
                         std::cos(2 * ky * y)) +
                    2 * (kx + ky) * std::cos(2 * kz * z) *
                      std::pow(std::sin(kx * x), 2) *
                      std::pow(std::sin(ky * y), 2)) +
                 std::pow(kz, 2) * std::cos(kx * x) * std::cos(2 * ky * y) *
                   std::pow(std::sin(kz * z), 2)) -
              kx * std::sin(kx * x) * std::sin(2 * ky * y) *
                std::sin(2 * kz * z) -
              std::cos(ky * y) *
                (ky * std::pow(std::cos(ky * y), 2) *
                   std::pow(std::sin(kx * x), 2) +
                 kx * std::pow(std::sin(ky * y), 2)) *
                std::pow(std::sin(2 * kz * z), 2))) /
             2. -
           kx * std::sin(kx * x) * std::sin(2 * kx * x) *
             (1.0 / std::tan(kx * x) * std::sin(2 * ky * y) *
                std::sin(2 * kz * z) +
              std::cos(ky * y) *
                (2 * kx * std::cos(2 * ky * y) * std::pow(std::cos(kz * z), 2) +
                 std::cos(kx * x) * std::pow(std::sin(ky * y), 2) *
                   std::pow(std::sin(2 * kz * z), 2)));
  }
...
mms()
{
  ...
  nodal_vector_workview<p, double> work_coords;
  auto& coords = work_coords.view();
  nodal_vector_workview<p, double> work_velocity;
  auto& velocity = work_velocity.view();
  nodal_scalar_workview<p, double> work_scalar;
  auto& scalar = work_scalar.view();

  // constants
  nodal_scalar_workview<p, double> work_diffusivity(1);
  auto& diffusivity = work_diffusivity.view();
  ...
  TestFunction test;
  nodal_scalar_workview<p, double> work_exact_resid(0);
  auto& exact_resid = work_exact_resid.view();
  ...
  nodal_scalar_workview<p, double> work_rhs(0);
  ...
  scs_vector_workview<p, double> work_diffmetric;
  auto& diffmetric = work_diffmetric.view();
  high_order_metrics::compute_laplacian_metric_linear(ops, coords, diffmetric);

  scs_scalar_workview<p, double> l_mdot(0);
  auto& mdot = l_mdot.view();
  high_order_metrics::compute_mdot_linear(
    ops, coords, diffmetric, 1, rho, velocity, Gp, pressure, mdot);
  ...
  for (int k = 1; k < p; ++k) {
    for (int j = 1; j < p; ++j) {
      for (int i = 1; i < p; ++i) {
        EXPECT_NEAR(0, rhs(k, j, i), 1.0e-8)
          << "k,j,i: " << k << ", " << j << ", " << i;
      }
    }
  }
