...
TEST_F(MomentumABLKernelHex8Mesh, NGP_abl_wall_func)
{
  ...
  for (int i=0; i < 12; i += 3) {
    EXPECT_NEAR(helperObjs.linsys->lhs_(i, i), lhsExact, 1.0e-12);
    EXPECT_NEAR(helperObjs.linsys->lhs_(i+1, i+1), lhsExact, 1.0e-12);
    EXPECT_NEAR(helperObjs.linsys->lhs_(i+2, i+2), 0.0, 1.0e-12);
  }
  ...
  for (int i=0; i < 12; ++i)
    for (int j=0; j < 12; ++j) {
      if (i == j) continue;
      EXPECT_NEAR(helperObjs.linsys->lhs_(i, j), 0.0, 1.0e-12);
    }

  // Check RHS
  for (int i=0; i < 12; ++i)
    EXPECT_NEAR(helperObjs.linsys->rhs_(i), rhsExact[i % 3], 1.0e-12);
}
