...
check_that_values_match(
  const sierra::kynema-ugf::SharedMemView<DoubleType*, SHMEM>& values,
...
copy_DoubleType0_to_double(
  const sierra::kynema-ugf::SharedMemView<DoubleType**, SHMEM>& view,
...
compare_old_scv_volume(
  const sierra::kynema-ugf::SharedMemView<DoubleType**, SHMEM>& v_coords,
  const sierra::kynema-ugf::SharedMemView<DoubleType*, SHMEM>& scv_volume,
  sierra::kynema-ugf::MasterElement* meSCV)
{
  int len = scv_volume.extent(0);
  std::vector<DoubleType> volume(len, 0.0);
  sierra::kynema-ugf::SharedMemView<DoubleType*, sierra::kynema-ugf::DeviceShmem> vol(
    volume.data(), volume.size());
  sierra::kynema-ugf::SharedMemView<DoubleType**, sierra::kynema-ugf::DeviceShmem> coords(
...
compare_old_scs_areav(
  const sierra::kynema-ugf::SharedMemView<DoubleType**, SHMEM>& v_coords,
  const sierra::kynema-ugf::SharedMemView<DoubleType**, SHMEM>& scs_areav,
  sierra::kynema-ugf::MasterElement* meSCS)
{
  int len = scs_areav.extent(0) * scs_areav.extent(1);
  std::vector<DoubleType> areav(len, 0.0);
  sierra::kynema-ugf::SharedMemView<DoubleType**, sierra::kynema-ugf::DeviceShmem> area(
...
compare_old_scs_grad_op(
  const sierra::kynema-ugf::SharedMemView<DoubleType**, SHMEM>& v_coords,
  const sierra::kynema-ugf::SharedMemView<DoubleType***, SHMEM>& scs_dndx,
  const sierra::kynema-ugf::SharedMemView<DoubleType***, SHMEM>& scs_deriv,
  sierra::kynema-ugf::MasterElement* meSCS)
{
  ...
  sierra::kynema-ugf::SharedMemView<DoubleType***, sierra::kynema-ugf::DeviceShmem> gradop(
    grad_op.data(), scs_dndx.extent(0), scs_dndx.extent(1), scs_dndx.extent(2));
  sierra::kynema-ugf::SharedMemView<DoubleType***, sierra::kynema-ugf::DeviceShmem> der(
...
compare_old_scs_gij(
  const sierra::kynema-ugf::SharedMemView<DoubleType**, SHMEM>& v_coords,
  const sierra::kynema-ugf::SharedMemView<DoubleType***, SHMEM>& v_gijUpper,
  const sierra::kynema-ugf::SharedMemView<DoubleType***, SHMEM>& v_gijLower,
  const sierra::kynema-ugf::SharedMemView<DoubleType***, SHMEM>& /* v_deriv */,
  sierra::kynema-ugf::MasterElement* meSCS)
{
  int gradOpLen =
  ...
  sierra::kynema-ugf::SharedMemView<DoubleType***, sierra::kynema-ugf::DeviceShmem> deriv(
...
void
test_ME_views(const std::vector<sierra::kynema-ugf::ELEM_DATA_NEEDED>& requests)
{
  ...
  for (sierra::kynema-ugf::ELEM_DATA_NEEDED request : requests) {
    driver.dataNeeded().add_master_element_call(
      request, sierra::kynema-ugf::CURRENT_COORDINATES);
  }
  ...
  driver.execute([&](
                   sierra::kynema-ugf::SharedMemData<
                     sierra::kynema-ugf::DeviceTeamHandleType,
                     sierra::kynema-ugf::DeviceShmem>& smdata) {
    // Extract data from scratchViews
    sierra::kynema-ugf::SharedMemView<DoubleType**, sierra::kynema-ugf::DeviceShmem>&
      v_coords =
        smdata.simdPrereqData.get_scratch_view_2D(*driver.coordinates_);
    auto& meViews =
      smdata.simdPrereqData.get_me_views(sierra::kynema-ugf::CURRENT_COORDINATES);

    if (meSCS != nullptr) {
      for (sierra::kynema-ugf::ELEM_DATA_NEEDED request : requests) {
        if (request == sierra::kynema-ugf::SCS_AREAV) {
          compare_old_scs_areav(v_coords, meViews.scs_areav, meSCS);
        }
        if (request == sierra::kynema-ugf::SCS_GRAD_OP) {
          compare_old_scs_grad_op(v_coords, meViews.dndx, meViews.deriv, meSCS);
        }
        if (request == sierra::kynema-ugf::SCS_SHIFTED_GRAD_OP) {
          compare_old_scs_shifted_grad_op(
            v_coords, meViews.dndx_shifted, meViews.deriv, meSCS);
        }
        if (request == sierra::kynema-ugf::SCS_GIJ) {
          compare_old_scs_gij(
    ...
    if (meSCV != nullptr) {
      for (sierra::kynema-ugf::ELEM_DATA_NEEDED request : requests) {
        if (request == sierra::kynema-ugf::SCV_VOLUME) {
          compare_old_scv_volume(v_coords, meViews.scv_volume, meSCV);
        }
        if (request == sierra::kynema-ugf::SCV_GRAD_OP) {
        ...
        if (request == sierra::kynema-ugf::SCV_SHIFTED_GRAD_OP) {
          if (AlgTraits::topo_ == stk::topology::HEX_8) {
            check_that_values_match(
...
{
  test_ME_views<sierra::kynema-ugf::AlgTraitsHex8>(
    {sierra::kynema-ugf::SCS_AREAV, sierra::kynema-ugf::SCS_GRAD_OP,
     sierra::kynema-ugf::SCV_VOLUME, sierra::kynema-ugf::SCV_GRAD_OP,
     sierra::kynema-ugf::SCV_SHIFTED_GRAD_OP});
}

TEST(KokkosME, test_tet4_views)
{
  test_ME_views<sierra::kynema-ugf::AlgTraitsTet4>(
    {sierra::kynema-ugf::SCS_AREAV, sierra::kynema-ugf::SCS_GRAD_OP,
     sierra::kynema-ugf::SCS_SHIFTED_GRAD_OP, sierra::kynema-ugf::SCV_VOLUME,
     sierra::kynema-ugf::SCV_GRAD_OP, sierra::kynema-ugf::SCV_SHIFTED_GRAD_OP});
}

TEST(KokkosME, test_tri32D_views)
{
  test_ME_views<sierra::kynema-ugf::AlgTraitsTri3_2D>(
    {sierra::kynema-ugf::SCS_AREAV, sierra::kynema-ugf::SCS_GRAD_OP,
     sierra::kynema-ugf::SCV_VOLUME});
}

TEST(KokkosME, test_tri32D_shifted_grad_op)
{
  test_ME_views<sierra::kynema-ugf::AlgTraitsTri3_2D>(
    {sierra::kynema-ugf::SCS_SHIFTED_GRAD_OP});
}

TEST(KokkosME, test_quad42D_views)
{
  test_ME_views<sierra::kynema-ugf::AlgTraitsQuad4_2D>(
    {sierra::kynema-ugf::SCS_AREAV, sierra::kynema-ugf::SCS_GRAD_OP,
     sierra::kynema-ugf::SCV_VOLUME});
}
...
{
  test_ME_views<sierra::kynema-ugf::AlgTraitsWed6>(
    {sierra::kynema-ugf::SCV_VOLUME, sierra::kynema-ugf::SCS_AREAV,
     sierra::kynema-ugf::SCS_GRAD_OP});
}
...
{
  test_ME_views<sierra::kynema-ugf::AlgTraitsPyr5>(
    {sierra::kynema-ugf::SCS_AREAV, sierra::kynema-ugf::SCS_GRAD_OP,
     sierra::kynema-ugf::SCV_VOLUME});
}
