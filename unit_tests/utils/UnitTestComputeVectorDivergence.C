...
TEST(utils, compute_vector_divergence)
{
  ...
  sierra::kynema-ugf::VectorFieldType* meshVec =
    &(realm.meta_data().declare_field<double>(
  ...
  sierra::kynema-ugf::ScalarFieldType* divV =
    &(realm.meta_data().declare_field<double>(
  ...
  sierra::kynema-ugf::GeometryAlgDriver geomAlgDriver(realm);
  geomAlgDriver.register_elem_algorithm<sierra::kynema-ugf::GeometryInteriorAlg>(
    sierra::kynema-ugf::INTERIOR, realm.meta_data().get_part("block_1"), "geometry");
  geomAlgDriver.register_face_algorithm<sierra::kynema-ugf::GeometryBoundaryAlg>(
    sierra::kynema-ugf::BOUNDARY, realm.meta_data().get_part("surface_1"),
    "geometry");
  ...
  sierra::kynema-ugf::compute_vector_divergence(
