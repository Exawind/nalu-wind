...
mixture_fraction_test_function(
  const stk::mesh::BulkData& bulk,
  const sierra::kynema-ugf::VectorFieldType& coordinates,
  const sierra::kynema-ugf::ScalarFieldType& mixtureFrac,
  const double znot,
  const double amf)
{
...
calc_mass_flow_rate(
  ...
  sierra::kynema-ugf::ScalarFieldType& massFlowRate)
{
  const auto& meta = bulk.mesh_meta_data();
  const int ndim = meta.spatial_dimension();
  EXPECT_EQ(ndim, 3);

  const sierra::kynema-ugf::ScalarFieldType& densityNp1 =
    density.field_of_state(stk::mesh::StateNP1);
  const sierra::kynema-ugf::VectorFieldType& velocityNp1 =
  ...
  const sierra::kynema-ugf::GenericFieldType& massFlowRate)
{
  ...
  dataReq.add_coordinates_field(
    coordinates, ndim, sierra::kynema-ugf::CURRENT_COORDINATES);
...
{
  using Traits = sierra::kynema-ugf::kynema-ugf_ngp::NGPMeshTraits<stk::mesh::NgpMesh>;
  using Quad4Traits = sierra::kynema-ugf::AlgTraitsQuad4;
  using ElemSimdDataType =
    sierra::kynema-ugf::kynema-ugf_ngp::ElemSimdData<stk::mesh::NgpMesh>;
  ...
  auto shape_fcn = sierra::kynema-ugf::shape_fcn<
    sierra::kynema-ugf::AlgTraitsQuad4, sierra::kynema-ugf::QuadRank::SCV>(
    sierra::kynema-ugf::use_shifted_quad(false));
  ...
  sierra::kynema-ugf::kynema-ugf_ngp::run_elem_algorithm(
    ...
    KOKKOS_LAMBDA(ElemSimdDataType & edata) {
      ...
      const auto& meViews =
        scrViews.get_me_views(sierra::kynema-ugf::CURRENT_COORDINATES);
  ...
  const sierra::kynema-ugf::VectorFieldType& edgeAreaVec)
{
  ...
  sierra::kynema-ugf::SharedMemView<double**> coords(w_coords.data(), npe, ndim);
  ...
  sierra::kynema-ugf::GenericFieldType& exposedAreaVec)
{
  ...
  const auto areaVecOps =
    sierra::kynema-ugf::kynema-ugf_ngp::simd_elem_field_updater(ngpMesh, areaVec);
