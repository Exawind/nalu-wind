...
calculate_mij_tensor(
  sierra::kynema-ugf::MasterElement& me, std::vector<double>& ws_coords)
{
  ...
  const sierra::kynema-ugf::SharedMemView<double**> coords(
...
test_metric_for_topo_2D(stk::topology topo, double tol)
{
  ...
  double gij_exact[4];
  sierra::kynema-ugf::mxm22(Q, Qt, gij_exact);

  // Compute eigendecomposition
  double mij_ev[2][2];
  double mij_evals[2][2];
  double (&gij_exact_pt)[2][2] = reinterpret_cast<double (&)[2][2]>(gij_exact);
  sierra::kynema-ugf::EigenDecomposition::sym_diagonalize(
  ...
  for (unsigned j = 0; j < topo.num_nodes(); ++j) {
    ...
    sierra::kynema-ugf::matvec22(Q, coords, &ws_coords[j * dim]);
  }
...
test_metric_for_topo_3D(stk::topology topo, double tol)
{
  ...
  double gij_exact[9];
  sierra::kynema-ugf::mxm33(Q, Qt, gij_exact);
  ...
  for (unsigned j = 0; j < topo.num_nodes(); ++j) {
    const double* coords = stk::mesh::field_data(coordField, nodes[j]);
    sierra::kynema-ugf::matvec33(Q, coords, &ws_coords[j * dim]);
  }
