...
using DeviceScalar =
  Kokkos::DualView<double*, Kokkos::LayoutRight, sierra::kynema-ugf::MemSpace>;

using DeviceVector =
  Kokkos::DualView<vs::Vector*, Kokkos::LayoutRight, sierra::kynema-ugf::MemSpace>;
...
test_vector_create_impl()
{
  ...
  ds.sync<sierra::kynema-ugf::MemSpace>();
  auto ddata = ds.template view<sierra::kynema-ugf::MemSpace>();
  Kokkos::parallel_for(
    1, KOKKOS_LAMBDA(int) {
  ...
  ds.modify<sierra::kynema-ugf::MemSpace>();
...
#define CHECK_ON_GPU(expr1, expr2)                                             \
  {                                                                            \
    ...
    ds.modify<DeviceScalar::host_mirror_space>();                              \
    ds.sync<sierra::kynema-ugf::MemSpace>();                                         \
    auto dv = ds.template view<sierra::kynema-ugf::MemSpace>();                      \
                                                                               \
    Kokkos::parallel_for(                                                      \
      1, KOKKOS_LAMBDA(int) {                                                  \
        auto v1 = expr1;                                                       \
...
test_device_lists_impl()
{
  ...
  auto dv = dvec.template view<sierra::kynema-ugf::MemSpace>();
  ...
  dvec.modify<sierra::kynema-ugf::MemSpace>();
