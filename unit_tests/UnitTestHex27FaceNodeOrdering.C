...
void
check_Hex27_creation(const stk::mesh::BulkData& bulk)
{
  ...
  for (stk::mesh::Entity elem : elems) {
    ...
    for (unsigned j = 0; j < bulk.num_faces(elem); ++j) {
      ...
      for (unsigned i = 0; i < bulk.num_nodes(face); ++i) {
        ...
        stk::mesh::Entity expectedNode =
          elem_node_rels[faceNodeOrdinals[ords[j]][i]];
...
void
check_Hex27_face_ip_node_ordering(const stk::mesh::BulkData& bulk)
{
  stk::topology hex27 = stk::topology::HEX_27;
  ...
  for (stk::mesh::Entity elem : elems) {
    ...
    for (unsigned j = 0; j < bulk.num_faces(elem); ++j) {
      ...
      for (int ip = 0; ip < quadSCS.num_integration_points(); ++ip) {
        ...
        stk::mesh::EntityId elemNearestNodeId =
          bulk.identifier(elem_node_rels[elemIpNearestNode]);
        stk::mesh::EntityId faceNearestNodeId =
          bulk.identifier(face_node_rels[faceIpNearestNode]);
