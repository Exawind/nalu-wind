...
TEST_F(ContinuityKernelHex8Mesh, NGP_continuity_gcl_node)
{
  ...
  helperObjs.nodeAlg->add_kernel<sierra::kynema-ugf::ContinuityGclNodeKernel>(*bulk_);
...
my_create(const T& hostObj)
{
  ...
  T* obj = sierra::kynema-ugf::kokkos_malloc_on_device<T>(debuggingName);
...
void
test_kernel_on_device(const sierra::kynema-ugf::ContinuityGclNodeKernel& kernel)
{
  sierra::kynema-ugf::ContinuityGclNodeKernel* deviceKernel = my_create(kernel);
}
...
TEST_F(ContinuityKernelHex8Mesh, NGP_continuity_gcl_node_kernel)
{
  ...
  sierra::kynema-ugf::ContinuityGclNodeKernel kernel(*bulk_);
