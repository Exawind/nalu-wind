  ...
  ScalarFieldType& wallNormDist)
{
  ...
  auto meFC = sierra::nalu::MasterElementRepo::get_surface_master_element_on_dev( FaceTraits::FaceTraits::topo_);
  auto meSCS = sierra::nalu::MasterElementRepo::get_surface_master_element_on_dev( FaceTraits::ElemTraits::topo_);
...
elem_loop_par_reduce(const stk::mesh::BulkData& bulk, ScalarFieldType& pressure)
{
  ...
  sierra::nalu::ElemDataRequests dataReq(meta);
  auto meSCV =
    sierra::nalu::MasterElementRepo::get_volume_master_element_on_dev(Hex8Traits::topo_);
  dataReq.add_cvfem_volume_me(meSCV);
