...
TEST(pyramid, is_in_element)
{
  ...
  auto dist = sierra::kynema-ugf::PyrSCS().isInElement(
...
check_interpolation_at_ips(
  ...
  sierra::kynema-ugf::MasterElement& me)
{
  ...
  sierra::kynema-ugf::MasterElement& me)
{
  ...
  sierra::kynema-ugf::SharedMemView<double***> gradop(
...
check_scv_shifted_ips_are_nodal(
  const stk::mesh::Entity* node_rels,
  const VectorFieldType& coordField,
  sierra::kynema-ugf::MasterElement& meSV)
{
  ...
  sierra::kynema-ugf::MasterElement& meSV)
{
  ...
  sierra::kynema-ugf::SharedMemView<double**> coords_mapped(
    ws_coords_mapped.data(), meSV.nodesPerElement_, dim);
  sierra::kynema-ugf::SharedMemView<double**> coords(
  ...
  for (int j = 0; j < meSV.nodesPerElement_; ++j) {
    ...
    if (dim == 3) {
      sierra::kynema-ugf::matvec33(QR.data(), coord, coord_mapped.data());
    } else {
      sierra::kynema-ugf::matvec22(QR.data(), coord, coord_mapped.data());
    }
  ...
  const double detQR = (dim == 3) ? sierra::kynema-ugf::determinant33(QR.data())
                                  : sierra::kynema-ugf::determinant22(QR.data());
  ASSERT_TRUE(detQR > 1.0e-15);

  double error = 0;
  std::vector<double> volume_integration_weights(meSV.num_integration_points());
  sierra::kynema-ugf::SharedMemView<double*> integration_weights(
  ...
  sierra::kynema-ugf::SharedMemView<double*> skewed_integration_weights(
...
void check_exposed_face_shifted_ips_are_nodal(
  const stk::mesh::Entity* node_rels,
  const VectorFieldType& coordField,
  sierra::kynema-ugf::MasterElement& meSS)
{
  // check that the subcontrol volume ips are at the nodes for the shifted ips
  ...
  sierra::kynema-ugf::MasterElement& me)
{
  ...
  bool isHexSCS = dynamic_cast<sierra::kynema-ugf::HexSCS*>(&me) != nullptr;
  bool isQuadSCS = dynamic_cast<sierra::kynema-ugf::Quad42DSCS*>(&me) != nullptr;
...
protected:
  ...
  void choose_topo(stk::topology topo)
  {
    ...
    meSV =
      sierra::kynema-ugf::MasterElementRepo::get_volume_master_element_on_host(topo);
  }
