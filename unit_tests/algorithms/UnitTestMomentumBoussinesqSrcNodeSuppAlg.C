...
TEST(TestMomentumBoussinesqRASrcNodeSuppAlg, single_value)
{
  ...
  *stk::mesh::field_data(temperature, node) = temperature_value;
  ...
  sierra::kynema-ugf::TimeIntegrator timeIntg;
  timeIntg.currentTime_ = 0;
  timeIntg.timeStepN_ = 1;
  sierra::kynema-ugf::MovingAveragePostProcessor avgPP(bulk, timeIntg, false);
  ...
  auto boussinesqRaAlg =
    sierra::kynema-ugf::MomentumBoussinesqRASrcNodeSuppAlg(helper.realm);
