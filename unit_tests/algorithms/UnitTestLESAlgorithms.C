...
{
  sierra::kynema-ugf::Realm& realm = this->create_realm();

  fill_mesh_and_init_fields();

  // Execute
  sierra::kynema-ugf::TurbViscSmagorinskyAlgorithm alg(realm, meshPart_);
  alg.execute();
...
TEST_F(TestTurbulenceAlgorithm, turbviscwalealgorithm)
{
  ...
  // Execute
  sierra::kynema-ugf::TurbViscWaleAlgorithm alg(realm, meshPart_);
  alg.execute();
