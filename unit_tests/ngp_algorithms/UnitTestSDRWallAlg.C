...
TEST_F(SSTKernelHex8Mesh, NGP_sdr_wall_lowRE)
{
  ...
  sierra::kynema-ugf::SDRWallFuncAlgDriver algDriver(helperObjs.realm);
  algDriver.register_face_elem_algorithm<sierra::kynema-ugf::SDRLowReWallAlg>(
    sierra::kynema-ugf::WALL, surfPart,
    sierra::kynema-ugf::get_elem_topo(helperObjs.realm, *surfPart), "sdr_lowre_wall",
    useShifted);
...

  {
    ...
    auto& ngpSdrBC =
      fieldMgr.get_field<double>(sdrWallbc_->mesh_meta_data_ordinal());
    auto& ngpWallArea =
      fieldMgr.get_field<double>(sdrWallArea_->mesh_meta_data_ordinal());
    ...
    const double wallFactor =
      realm.get_turb_model_constant(sierra::kynema-ugf::TM_SDRWallFactor);
    const double betaOne =
      realm.get_turb_model_constant(sierra::kynema-ugf::TM_betaOne);
...
TEST_F(SSTKernelHex8Mesh, NGP_sdr_wall_func)
{
  ...
  auto* surfPart = part->subsets()[0];
  sierra::kynema-ugf::SDRWallFuncAlgDriver algDriver(helperObjs.realm);
  algDriver.register_face_elem_algorithm<sierra::kynema-ugf::SDRWallFuncAlg>(
    sierra::kynema-ugf::WALL, surfPart,
    sierra::kynema-ugf::get_elem_topo(helperObjs.realm, *surfPart), "sdr_lowre_wall");
...

  {
    ...
    const double kappa = realm.get_turb_model_constant(sierra::kynema-ugf::TM_kappa);
    const double sqrtBetaStar =
      std::sqrt(realm.get_turb_model_constant(sierra::kynema-ugf::TM_betaStar));
