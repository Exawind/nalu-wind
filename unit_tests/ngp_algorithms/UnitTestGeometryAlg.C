  ...
  // Exposed area vector check
  {
    ...
    for (const auto* b : bkts)
      for (const auto face : *b) {
        ...
        for (int ip = 0; ip < sierra::kynema_ugf::AlgTraitsQuad4::numFaceIp_; ++ip) {
          double aMagSqr = 0.0;
          for (int i = 0; i < 3; i++) {
            const double av =
...
TEST_F(KsgsKernelHex8Mesh, NGP_geometry_wall_func)
{
  ...
  auto* surfPart = part->subsets()[0];
  sierra::kynema_ugf::GeometryAlgDriver geomAlgDriver(helperObjs.realm);
  geomAlgDriver.register_wall_func_algorithm<sierra::kynema_ugf::WallFuncGeometryAlg>(
    sierra::kynema_ugf::WALL, surfPart,
    sierra::kynema_ugf::get_elem_topo(helperObjs.realm, *surfPart), "geometry");
