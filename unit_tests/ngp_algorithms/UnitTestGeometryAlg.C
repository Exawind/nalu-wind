...
TEST_F(TestKernelHex8Mesh, NGP_geometry_bndry)
{
  ...
  geomAlgDriver.register_face_algorithm<sierra::kynema-ugf::GeometryBoundaryAlg>(
    sierra::kynema-ugf::WALL, surfPart, "geometry");
  ...
  // Exposed area vector check
  {
    ...
    for (const auto* b : bkts)
      for (const auto face : *b) {
        ...
        for (int ip = 0; ip < sierra::kynema-ugf::AlgTraitsQuad4::numFaceIp_; ++ip) {
          double aMagSqr = 0.0;
          for (int i = 0; i < 3; i++) {
            const double av =
              areaVec[ip * sierra::kynema-ugf::AlgTraitsQuad4::nDim_ + i];
            aMagSqr += av * av;
          }
...
TEST_F(KsgsKernelHex8Mesh, NGP_geometry_wall_func)
{
  ...
  auto* surfPart = part->subsets()[0];
  sierra::kynema-ugf::GeometryAlgDriver geomAlgDriver(helperObjs.realm);
  geomAlgDriver.register_wall_func_algorithm<sierra::kynema-ugf::WallFuncGeometryAlg>(
    sierra::kynema-ugf::WALL, surfPart,
    sierra::kynema-ugf::get_elem_topo(helperObjs.realm, *surfPart), "geometry");
