...
TEST_F(MomentumEdgeHex8Mesh, NGP_mdot_calc_edge)
{
  ...
  sierra::kynema-ugf::MdotEdgeAlg mdotAlg(helperObjs.realm, partVec_[0]);
  mdotAlg.execute();
...
TEST_F(MomentumEdgeHex8Mesh, NGP_mdot_rho_accum)
{
  ...
  sierra::kynema-ugf::MdotAlgDriver mdotDriver(
    helperObjs.realm, elementContinuityEqs);

  mdotDriver.register_elem_algorithm<sierra::kynema-ugf::MdotDensityAccumAlg>(
    sierra::kynema-ugf::INTERIOR, partVec_[0], "mdot_rho_acc", mdotDriver,
    lumpedMass);

  mdotDriver.execute();

  const double expectedValue =
    1.0 * sierra::kynema-ugf::AlgTraitsHex8::nodesPerElement_;
...
TEST_F(MomentumEdgeHex8Mesh, NGP_mdot_open_correction)
{
  ...
  mdotDriver.register_open_mdot_corrector_alg(
    sierra::kynema-ugf::OPEN, surfPart, "mdot_open_correction");
...
TEST_F(MomentumEdgeHex8Mesh, NGP_mdot_inflow)
{
  ...
  sierra::kynema-ugf::MdotAlgDriver mdotDriver(
    helperObjs.realm, elementContinuityEqs);
  mdotDriver.register_face_algorithm<sierra::kynema-ugf::MdotInflowAlg>(
    sierra::kynema-ugf::INFLOW, surfPart, "mdot_inflow", mdotDriver, useShifted);
  mdotDriver.execute();
...
TEST_F(MomentumEdgeHex8Mesh, NGP_mdot_open_edge)
{
  ...
  sierra::kynema-ugf::MdotAlgDriver mdotDriver(
    helperObjs.realm, elementContinuityEqs);
  mdotDriver.register_open_mdot_algorithm<sierra::kynema-ugf::MdotOpenEdgeAlg>(
    sierra::kynema-ugf::OPEN, surfPart, stk::topology::HEX_8, "mdot_open",
