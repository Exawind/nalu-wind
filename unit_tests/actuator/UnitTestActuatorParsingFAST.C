...
YAML::Node create_yaml_node(const std::vector<std::string>& testFile){
  std::string temp;
...
void test_wo_lines(const std::vector<std::string>& testFile, const ActuatorMeta& actMeta){
  ...
  for(unsigned i = 0; i<testFile.size(); i++){
    ...
    try{
      ...
      try{
      EXPECT_THROW(actuator_FAST_parse(y_node, actMeta), std::runtime_error)<<" when missing: "
          <<testFile[i];
      }
      catch(std::exception const& err){ // yaml or some other error sliped through
        EXPECT_TRUE(false) << "Missing line: "<< testFile[i]<<"Leads to exception: "<< err.what();
      }
    }
    catch(std::exception const& err){ //yaml-error throws during node creation
      //Do nothing for now, but should create a wrapper to catch these errors and make them more helpful
      //EXPECT_TRUE(false) << "Missing line: "<< testFile[i]<<"Leads to exception: "<< err.what();
    }
  }
  std::cout.rdbuf(sbuf);
