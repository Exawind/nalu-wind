...
{
  auto* me = sierra::nalu::MasterElementRepo::get_surface_master_element_on_host(topo);
  ...
  for (unsigned side_ordinal = 0; side_ordinal < topo.num_sides();
    ...
    for (int n = 0; n < numSideNodes; ++n) {
      EXPECT_EQ(
        static_cast<int>(topoSideNodeOrdinals[n]), meSideNodeOrdinals[n]);
    }
  }
}
} // namespace
...
TEST_ALL_TOPOS(
  MasterElementSideNodeOrdinals, side_node_ordinals_are_same_as_stk)
