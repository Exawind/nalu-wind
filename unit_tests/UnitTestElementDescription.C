...
TEST(ElementDescriptionTestHex, node_map_P2)
{
  ...
  std::vector<int> stkNodeMap = {0,  8,  1,  11, 20, 9,  3,  10, 2,
                                 12, 24, 13, 22, 26, 23, 15, 25, 14,
                                 4,  16, 5,  19, 21, 17, 7,  18, 6};

  auto elem = sierra::kynema-ugf::HexNElementDescription(2);
...
TEST(ElementDescriptionTestHex, node_map_P3)
{
  ...
  auto elem = sierra::kynema-ugf::HexNElementDescription(3);
...
TEST(ElementDescriptionTestHex, side_ordinal_map_P2)
{
  std::vector<std::vector<int>> sideNodeMap = {
    {0, 1, 5, 4, 8, 13, 16, 12, 24},  {1, 2, 6, 5, 9, 14, 17, 13, 23},
