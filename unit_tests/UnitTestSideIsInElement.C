...
void
check_side_is_in_element(stk::topology topo)
{
  ...
  std::uniform_real_distribution<double> random_pt(
    -0.1, 1.1); // can be outside of element side
  ...
  for (int ntrials = 0; ntrials < 25; ++ntrials) {
    ...
    randomly_perturb_element_coords(
      rng, topo.num_nodes(), bulk->begin_nodes(elem), coordField);
    ...
    for (const auto* ib : face_buckets) {
      ...
      for (size_t k = 0; k < b.size(); ++k) {
        ...
        meSide->isInElement(
          faceCoords.data(), faceInterpCoords.data(), isoFacePoint.data());
