...
struct EntityIdVectorHash
{
  std::size_t operator()(const stk::mesh::EntityIdVector& ids) const
  {
    ...
    for (unsigned j = 1; j < ids.size(); ++j) {
      hash ^= std::hash<std::size_t>()(ids[j]) + shift + (hash << 6) + (hash >> 2);
    }
    return hash;
  }
};
...
void create_nodes_for_connectivity_map(stk::mesh::BulkData& bulk, const ConnectivityMap& edgeConnectivity);
...
NodesElemMap make_base_nodes_to_elem_map_at_boundary(
  const HexNElementDescription& desc, const stk::mesh::BulkData& mesh, const stk::mesh::PartVector& meshParts);

std::unordered_map<stk::mesh::Entity, stk::mesh::Entity> exposed_side_to_super_elem_map(
  const HexNElementDescription& desc, const stk::mesh::BulkData& bulk, const stk::mesh::PartVector& base_elem_mesh_parts);

stk::mesh::PartVector
create_boundary_elements(int p, stk::mesh::BulkData& bulk, const stk::mesh::PartVector& parts);
