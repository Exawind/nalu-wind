...
template <int p, typename Scalar = DoubleType>
using scs_symmtensor_workview =
  typename CVFEMViews<p, Scalar>::scs_symmtensor_wsv;
...
template <typename ArrayType>
using CoeffViewType = Kokkos::
  View<ArrayType, Kokkos::LayoutRight, Kokkos::MemoryTraits<Kokkos::Aligned>>;
...
template <int p, typename Scalar = DoubleType>
using linear_nodal_matrix_view =
  CoeffViewType<linear_nodal_matrix_array<p, Scalar>>;
//--------------------------------------------------------------------------
template <int p, typename Scalar = DoubleType>
using linear_scs_matrix_array = Scalar[2][p];

template <int p, typename Scalar = DoubleType>
using linear_scs_matrix_view =
  CoeffViewType<linear_scs_matrix_array<p, Scalar>>;
//--------------------------------------------------------------------------
...
#define DeclareCVFEMTypeDefs(x)                                                \
  CVFEMTypeDefsDim(x, nodal, scalar) CVFEMTypeDefsDim(x, nodal, vector)        \
    CVFEMTypeDefsDim(x, nodal, tensor) CVFEMTypeDefsDim(x, scs, scalar)        \
      CVFEMTypeDefsDim(x, scs, vector) CVFEMTypeDefsDim(x, scs, symmtensor)    \
        CVFEMTypeDefsDim(x, scs, tensor) using matrix##_##view =               \
