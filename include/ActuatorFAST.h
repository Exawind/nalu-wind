...
/** @file ActuatorFAST.h
 *  @brief A class to couple Nalu with OpenFAST for actuator simulations of wind turbines
 *
 */
...
/** The ActuatorFAST class couples Nalu with the third party library OpenFAST for actuator simulations of wind turbines
 *
 ...
 * National Renewable Energy Laboratory (NREL). The ActuatorFAST class will help Nalu
 * effectively act as an inflow module to OpenFAST by supplying the velocity field information.
 * The effect of the turbine on the flow field is modeled using the actuator approach.
 ...
 *    processors controlling > 0 turbines initialize OpenFAST, populate the map of ``ActuatorPointInfo``
 *    and initialize element searches for all the actuator points associated with the turbines. For every actuator point,
 *    the elements within a specified search radius are found and stored in the corresponding object of the
 ...
 *    ``ActuatorPointInfoMap`` to assemble source terms. For each node \f$n\f$within the
