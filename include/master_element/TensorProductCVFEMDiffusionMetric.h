...
void
compute_laplacian_metric_linear(
    ...
    for (int j = 0; j < p + 1; ++j) {
      ...
      for (int i = 0; i < p; ++i) {
        ...
        const Scalar inv_detj =
          1.0 / (jac[0][0] * adjJac[0][0] + jac[1][0] * adjJac[1][0] +
                 jac[2][0] * adjJac[2][0]);

        metric(XH, k, j, i, XH) = -inv_detj * (adjJac[XH][XH] * adjJac[XH][XH] +
                                               adjJac[XH][YH] * adjJac[XH][YH] +
                                               adjJac[XH][ZH] * adjJac[XH][ZH]);
        metric(XH, k, j, i, YH) = -inv_detj * (adjJac[XH][XH] * adjJac[YH][XH] +
                                               adjJac[XH][YH] * adjJac[YH][YH] +
                                               adjJac[XH][ZH] * adjJac[YH][ZH]);
        metric(XH, k, j, i, ZH) = -inv_detj * (adjJac[XH][XH] * adjJac[ZH][XH] +
                                               adjJac[XH][YH] * adjJac[ZH][YH] +
                                               adjJac[XH][ZH] * adjJac[ZH][ZH]);
      }
    }
  }
  ...
  for (int k = 0; k < p + 1; ++k) {
    ...
    for (int j = 0; j < p; ++j) {
      ...
      for (int i = 0; i < p + 1; ++i) {
        ...
        metric(YH, k, j, i, XH) = -inv_detj * (adjJac[YH][XH] * adjJac[XH][XH] +
                                               adjJac[YH][YH] * adjJac[XH][YH] +
                                               adjJac[YH][ZH] * adjJac[XH][ZH]);
        metric(YH, k, j, i, YH) = -inv_detj * (adjJac[YH][XH] * adjJac[YH][XH] +
                                               adjJac[YH][YH] * adjJac[YH][YH] +
                                               adjJac[YH][ZH] * adjJac[YH][ZH]);
        metric(YH, k, j, i, ZH) = -inv_detj * (adjJac[YH][XH] * adjJac[ZH][XH] +
                                               adjJac[YH][YH] * adjJac[ZH][YH] +
                                               adjJac[YH][ZH] * adjJac[ZH][ZH]);
      }
    }
  }
  ...
  for (int k = 0; k < p; ++k) {
    ...
    for (int j = 0; j < p + 1; ++j) {
      ...
      for (int i = 0; i < p + 1; ++i) {
        ...
        metric(ZH, k, j, i, XH) = -inv_detj * (adjJac[ZH][XH] * adjJac[XH][XH] +
                                               adjJac[ZH][YH] * adjJac[XH][YH] +
                                               adjJac[ZH][ZH] * adjJac[XH][ZH]);
        metric(ZH, k, j, i, YH) = -inv_detj * (adjJac[ZH][XH] * adjJac[YH][XH] +
                                               adjJac[ZH][YH] * adjJac[YH][YH] +
                                               adjJac[ZH][ZH] * adjJac[YH][ZH]);
        metric(ZH, k, j, i, ZH) = -inv_detj * (adjJac[ZH][XH] * adjJac[ZH][XH] +
                                               adjJac[ZH][YH] * adjJac[ZH][YH] +
                                               adjJac[ZH][ZH] * adjJac[ZH][ZH]);
      }
    }
  }
}
...
template <int p, typename Scalar>
void
compute_laplacian_metric_lineart(
  const CVFEMOperators<p, Scalar>& ops,
  const nodal_vector_view<p, Scalar>& xc,
  scs_vector_view<p, Scalar>& metric)
{
...
template <int p, typename Scalar>
void
compute_diffusion_metric_linear(
  const CVFEMOperators<p, Scalar>& ops,
  const nodal_vector_view<p, Scalar>& xc,
  ...
  for (int k = 0; k < p + 1; ++k) {
    ...
    for (int j = 0; j < p + 1; ++j) {
      ...
      for (int i = 0; i < p; ++i) {
        ...
        const Scalar inv_detj = diffIp(k, j, i) / (jac[0][0] * adjJac[0][0] +
                                                   jac[1][0] * adjJac[1][0] +
                                                   jac[2][0] * adjJac[2][0]);
