...
cofactorMatrix(ftype adjJac[][3], const ftype jact[][3])
{
...
template <typename ftype>
KOKKOS_INLINE_FUNCTION void
cofactorMatrix(ftype adjJac[][2], const ftype jact[][2])
{
  adjJac[0][0] = jact[1][1];
  adjJac[0][1] = -jact[1][0];
  adjJac[1][0] = -jact[0][1];
  ...
  for (unsigned ip = 0; ip < referenceGradWeights.extent(0); ++ip) {
    ...
    NALU_ALIGNED const ftype inv_detj =
      ...
      (gup(ip, 0, 0) *
       ...
       gup(ip, 0, 1) *
         (gup(ip, 0, 1) * gup(ip, 2, 2) - gup(ip, 1, 2) * gup(ip, 0, 2)) +
       gup(ip, 0, 2) *
         (gup(ip, 0, 1) * gup(ip, 1, 2) - gup(ip, 1, 1) * gup(ip, 0, 2)));
  ...
  double* metric)
{
  ...
  for (int ki = 0; ki < nint; ++ki) {
    ...
    for (int kn = 0; kn < npe; ++kn) {
      dx_ds[0][0] += deriv[(ki * npe + kn) * ndim + 0] * coords[kn * ndim + 0];
      dx_ds[0][1] += deriv[(ki * npe + kn) * ndim + 1] * coords[kn * ndim + 0];
      dx_ds[1][0] += deriv[(ki * npe + kn) * ndim + 0] * coords[kn * ndim + 1];
      dx_ds[1][1] += deriv[(ki * npe + kn) * ndim + 1] * coords[kn * ndim + 1];
    }
    ...
    const double trace =
      metric[(ki * ndim + 0) * ndim + 0] + metric[(ki * ndim + 1) * ndim + 1];
    const double det =
      metric[(ki * ndim + 0) * ndim + 0] * metric[(ki * ndim + 1) * ndim + 1] -
      metric[(ki * ndim + 0) * ndim + 1] * metric[(ki * ndim + 1) * ndim + 0];
  ...
  OutputViewType& metric)
{
  ...
  for (int ki = 0; ki < nint; ++ki) {
    ...
    const ftype det =
      metric(ki, 0, 0) * metric(ki, 1, 1) - metric(ki, 0, 1) * metric(ki, 1, 0);
  ...
  double* metric)
{
  ...
  for (int ip = 0; ip < numIntPoints; ++ip) {
    ...
    for (int n = 0; n < AlgTraits::nodesPerElement_; ++n) {
      jac[0][0] +=
        deriv[(ip * AlgTraits::nodesPerElement_ + n) * AlgTraits::nDim_ + 0] *
        coords[n * AlgTraits::nDim_ + 0];
      jac[0][1] +=
        deriv[(ip * AlgTraits::nodesPerElement_ + n) * AlgTraits::nDim_ + 1] *
        coords[n * AlgTraits::nDim_ + 0];
      jac[0][2] +=
        deriv[(ip * AlgTraits::nodesPerElement_ + n) * AlgTraits::nDim_ + 2] *
        coords[n * AlgTraits::nDim_ + 0];

      jac[1][0] +=
        deriv[(ip * AlgTraits::nodesPerElement_ + n) * AlgTraits::nDim_ + 0] *
        coords[n * AlgTraits::nDim_ + 1];
      jac[1][1] +=
        deriv[(ip * AlgTraits::nodesPerElement_ + n) * AlgTraits::nDim_ + 1] *
        coords[n * AlgTraits::nDim_ + 1];
      jac[1][2] +=
    ...
    // Now we take the sqrt(M^2) using eigenvalue decomposition, i.e. M = A
    // sqrt(L) A^T where M^2 = A L A^T since M^2 is symmetric positive definite
    // as is M
    double Q[3][3];
    double D[3][3];
    ...
    for (unsigned i = 0; i < 3; i++) {
      for (unsigned j = 0; j < 3; j++) {
        metric(ip, i, j) = Q[i][0] * Q[j][0] * stk::math::sqrt(D[0][0]) +
                           Q[i][1] * Q[j][1] * stk::math::sqrt(D[1][1]) +
                           Q[i][2] * Q[j][2] * stk::math::sqrt(D[2][2]);
        // std::cerr << metric(ip,i,j) << " " << Q[i][2] << " " << " " <<
        // Q[j][2] << " " << D[2][2] << std::endl;
      }
    }
  }
}
