...
nodal_matrix_view<p, Scalar>
nodal_integration_weights(const double* /* nodeLocs */, const double* scsLocs)
{
...
template <int p, typename Scalar = DoubleType>
nodal_matrix_view<p, Scalar>
lumped_nodal_integration_weights(const double* nodeLocs, const double* scsLocs)
{
  constexpr int nodes1D = p + 1;
  ...
  nodal_matrix_view<p, Scalar> lumped_weight_matrix(
    "lumped_nodal_integration_weights");
...
template <int p, typename Scalar = DoubleType>
scs_matrix_view<p, Scalar>
scs_interpolation_weights(const double* nodeLocs, const double* scsLocs)
{
  constexpr int nodes1D = p + 1;
  scs_matrix_view<p, Scalar> scsInterp(
    "subcontrol surface interpolation matrix");
...
template <int p, typename Scalar = DoubleType>
scs_matrix_view<p, Scalar>
scs_derivative_weights(const double* nodeLocs, const double* scsLocs)
{
  constexpr int nodes1D = p + 1;
...
template <int p, typename Scalar = DoubleType>
linear_scs_matrix_view<p, Scalar>
...
template <int p, typename Scalar = DoubleType>
linear_nodal_matrix_view<p, Scalar>
...
lumped_nodal_integration_weights()
{
  ...
  return lumped_nodal_integration_weights<p, Scalar>(
    nodeLocs.data(), scsLocs.data());
}
