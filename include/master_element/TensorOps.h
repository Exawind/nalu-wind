...
KOKKOS_FORCEINLINE_FUNCTION ScalarType
vecnorm_sq2(const ScalarType* x)
{
...
template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION ScalarType
vecnorm_sq3(const ScalarType* x)
{
...
template <typename ScalarType>
ScalarType
ddot(const ScalarType* u, const ScalarType* v, int n)
{
  ScalarType val = 0.0;
  for (int i = 0; i < n; ++i) {
    val += u[i] * v[i];
...
template <typename ScalarType>
void
cross3(const ScalarType* u, const ScalarType* v, ScalarType* cross)
{
...
template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION ScalarType
determinant22(const ScalarType* mat)
{
...
template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION ScalarType
determinant33(const ScalarType* mat)
{
  enum {
    XX = 0,
    XY = 1,
  ...
  return (
    mat[XX] * (mat[YY] * mat[ZZ] - mat[YZ] * mat[ZY]) -
...
adjugate_matrix33(const ScalarType jact[3][3], ScalarType adjJac[3][3])
{
...
template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION void
invert_matrix33(const ScalarType A[3][3], ScalarType Ainv[3][3])
{
...
KOKKOS_FORCEINLINE_FUNCTION void
invert_matrix33(
  const ScalarType* POINTER_RESTRICT A, ScalarType* POINTER_RESTRICT Ainv)
{
  enum {
    XX = 0,
    XY = 1,
...
enum class SolveStatus { FAILED = 0, SUCCEEDED = 1 };

template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION SolveStatus
solve22(
...
{
  if (!stk::simd::are_any(
        stk::math::abs(determinant22(A)) > tiny_positive_value())) {
...
{
  if (!stk::simd::are_any(
        stk::math::abs(determinant33(A)) > tiny_positive_value())) {
    return SolveStatus::FAILED;
  }
...
template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION void
transpose22(const ScalarType* A, ScalarType* At)
{
  At[0] = A[0];
  At[1] = A[2];
  At[2] = A[1];
...
template <typename ScalarType>
KOKKOS_FORCEINLINE_FUNCTION void
transpose33(const ScalarType* A, ScalarType* At)
{
  enum {
    XX = 0,
    XY = 1,
