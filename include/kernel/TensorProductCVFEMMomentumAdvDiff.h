    ...
    for (int m = 0; m < n1D; ++m) {
      ...
      for (int k = 0; k < n1D; ++k) {
        ...
        for (int j = 0; j < n1D; ++j) {
          ...
          for (int q = 0; q < n1D; ++q) {
            non_orth_y += mat.nodalWeights(n, k) * mat.nodalWeights(m, q) *
                          mat.nodalDeriv(q, j) * metric(XH, k, q, l_minus, YH);
            non_orth_z += mat.nodalWeights(n, q) * mat.nodalWeights(m, j) *
      ...
      for (int l = 1; l < n1D - 1; ++l) {
        for (int k = 0; k < n1D; ++k) {
          ...
          for (int j = 0; j < n1D; ++j) {
            ...
            for (int q = 0; q < n1D; ++q) {
              const Scalar wy = mat.nodalWeights(n, k) *
                                mat.nodalWeights(m, q) * mat.nodalDeriv(q, j);
              non_orth_y_m1 += wy * metric(XH, k, q, l - 1, YH);
              non_orth_y_p0 += wy * metric(XH, k, q, l + 0, YH);

              const Scalar wz = mat.nodalWeights(n, q) *
                                mat.nodalWeights(m, j) * mat.nodalDeriv(q, k);
            ...
            for (int i = 0; i < n1D; ++i) {
              const Scalar integrated_flux_m =
                ...
                mat.scsInterp(l - 1, i) *
                  (-advm1 + non_orth_y_m1 + non_orth_z_m1);
              const Scalar integrated_flux_p =
                orthp0 * mat.scsDeriv(l + 0, i) +
                mat.scsInterp(l + 0, i) *
                  (-advp0 + non_orth_y_p0 + non_orth_z_p0);
    ...
    for (int l = 0; l < n1D; ++l) {
      ...
      for (int k = 0; k < n1D; ++k) {
        ...
        for (int i = 0; i < n1D; ++i) {
          ...
          Scalar non_orth_z = 0.0;
          for (int q = 0; q < n1D; ++q) {
            non_orth_x += mat.nodalWeights(n, k) * mat.nodalWeights(l, q) *
                          mat.nodalDeriv(q, i) * metric(YH, k, m_minus, q, XH);
            non_orth_z += mat.nodalWeights(n, q) * mat.nodalWeights(l, i) *
      ...
      for (int m = 1; m < n1D - 1; ++m) {
        for (int k = 0; k < n1D; ++k) {
          ...
          for (int i = 0; i < n1D; ++i) {
            ...
            for (int q = 0; q < n1D; ++q) {
              const Scalar wx = mat.nodalWeights(n, k) *
                                mat.nodalWeights(l, q) * mat.nodalDeriv(q, i);
              non_orth_x_m1 += wx * metric(YH, k, m - 1, q, XH);
              non_orth_x_p0 += wx * metric(YH, k, m + 0, q, XH);

              const Scalar wz = mat.nodalWeights(n, q) *
                                mat.nodalWeights(l, i) * mat.nodalDeriv(q, k);
            ...
            for (int j = 0; j < n1D; ++j) {
              const Scalar integrated_flux_m =
                ...
                mat.scsInterp(m - 1, j) *
                  (-advm1 + non_orth_x_m1 + non_orth_z_m1);
              const Scalar integrated_flux_p =
                orthp0 * mat.scsDeriv(m + 0, j) +
                mat.scsInterp(m + 0, j) *
                  (-advp0 + non_orth_x_p0 + non_orth_z_p0);
              const Scalar flux_diff = integrated_flux_p - integrated_flux_m;
              lhs(idx<n1D>(XH, n, m, l), idx<n1D>(XH, k, j, i)) += flux_diff;
              lhs(idx<n1D>(YH, n, m, l), idx<n1D>(YH, k, j, i)) += flux_diff;
              lhs(idx<n1D>(ZH, n, m, l), idx<n1D>(ZH, k, j, i)) += flux_diff;
    ...
    for (int l = 0; l < n1D; ++l) {
      ...
      for (int j = 0; j < n1D; ++j) {
        ...
        for (int i = 0; i < n1D; ++i) {
          ...
          Scalar non_orth_y = 0.0;
          for (int q = 0; q < n1D; ++q) {
            non_orth_x += mat.nodalWeights(m, j) * mat.nodalWeights(l, q) *
                          mat.nodalDeriv(q, i) * metric(ZH, n_minus, j, q, XH);
            non_orth_y += mat.nodalWeights(m, q) * mat.nodalWeights(l, i) *
      ...
      for (int n = 1; n < n1D - 1; ++n) {
        for (int j = 0; j < n1D; ++j) {
          ...
          for (int i = 0; i < n1D; ++i) {
            ...
            for (int q = 0; q < n1D; ++q) {
              const Scalar wx = mat.nodalWeights(m, j) *
                                mat.nodalWeights(l, q) * mat.nodalDeriv(q, i);
              non_orth_x_m1 += wx * metric(ZH, n - 1, j, q, XH);
              non_orth_x_p0 += wx * metric(ZH, n + 0, j, q, XH);

              const Scalar wy = mat.nodalWeights(m, q) *
                                mat.nodalWeights(l, i) * mat.nodalDeriv(q, j);
            ...
            for (int k = 0; k < n1D; ++k) {
              const Scalar integrated_flux_m =
                ...
                mat.scsInterp(n - 1, k) *
                  (-advm1 + non_orth_x_m1 + non_orth_y_m1);
              const Scalar integrated_flux_p =
                orthp0 * mat.scsDeriv(n + 0, k) +
                mat.scsInterp(n + 0, k) *
                  (-advp0 + non_orth_x_p0 + non_orth_y_p0);
...
void
area_weighted_face_normal_shear_stress(
      ...
      for (int i = 0; i < p; ++i) {
        ...
        for (int d = 0; d < 3; ++d) {
          local_grad[d][XH] = adjJac[XH][XH] * gradu_scs(k, j, i, d, XH) +
                              ...
                              adjJac[XH][ZH] * gradu_scs(k, j, i, d, ZH);
          local_grad[d][YH] = adjJac[YH][XH] * gradu_scs(k, j, i, d, XH) +
                              adjJac[YH][YH] * gradu_scs(k, j, i, d, YH) +
                              adjJac[YH][ZH] * gradu_scs(k, j, i, d, ZH);
          local_grad[d][ZH] = adjJac[ZH][XH] * gradu_scs(k, j, i, d, XH) +
                              adjJac[ZH][YH] * gradu_scs(k, j, i, d, YH) +
        ...
        const Scalar inv_detj =
          2 * visc_scs(k, j, i) /
          (jact[0][0] * adjJac[0][0] + jact[1][0] * adjJac[1][0] +
           jact[2][0] * adjJac[2][0]);
  ...
  nodal_vector_view<poly_order, Scalar>& rhs)
{
  ...
  for (int k = 0; k < n1D; ++k) {
    for (int j = 0; j < n1D; ++j) {
      for (int i = 0; i < nscs; ++i) {
        integrand(k, j, i, XH) =
          tau_dot_a(XH, k, j, i, XH) - mdot(XH, k, j, i) * vel_scs(k, j, i, XH);
        integrand(k, j, i, YH) =
          tau_dot_a(XH, k, j, i, YH) - mdot(XH, k, j, i) * vel_scs(k, j, i, YH);
        integrand(k, j, i, ZH) =
          tau_dot_a(XH, k, j, i, ZH) - mdot(XH, k, j, i) * vel_scs(k, j, i, ZH);
      }
    }
  }
  ...
  for (int k = 0; k < n1D; ++k) {
    for (int j = 0; j < nscs; ++j) {
      for (int i = 0; i < n1D; ++i) {
        integrand(k, j, i, XH) =
          tau_dot_a(YH, k, j, i, XH) - mdot(YH, k, j, i) * vel_scs(k, j, i, XH);
        integrand(k, j, i, YH) =
          tau_dot_a(YH, k, j, i, YH) - mdot(YH, k, j, i) * vel_scs(k, j, i, YH);
        integrand(k, j, i, ZH) =
          tau_dot_a(YH, k, j, i, ZH) - mdot(YH, k, j, i) * vel_scs(k, j, i, ZH);
      }
    }
  }
  ...
  for (int k = 0; k < nscs; ++k) {
    for (int j = 0; j < n1D; ++j) {
      for (int i = 0; i < n1D; ++i) {
        integrand(k, j, i, XH) =
          tau_dot_a(ZH, k, j, i, XH) - mdot(ZH, k, j, i) * vel_scs(k, j, i, XH);
        integrand(k, j, i, YH) =
          tau_dot_a(ZH, k, j, i, YH) - mdot(ZH, k, j, i) * vel_scs(k, j, i, YH);
        integrand(k, j, i, ZH) =
          tau_dot_a(ZH, k, j, i, ZH) - mdot(ZH, k, j, i) * vel_scs(k, j, i, ZH);
      }
    }
  }
