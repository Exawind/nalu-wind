...
public:
  ...
  static void filter_to_nearest(
    EntityKeyMap& RangeToDomain, const MeshA& FromElem, MeshB& ToPoints);

  static void apply(
    MeshB& ToPoints, const MeshA& FromElem, const EntityKeyMap& RangeToDomain);
};
  ...
  EntityKeyMap& RangeToDomain, const MeshA& FromElem, MeshB& ToPoints)
{
  ...
  NaluEnv::self().naluOutputP0() << "  Maximum number of candidate bounding "
  ...
  for (ii = RangeToDomain.begin(); ii != RangeToDomain.end(); ++ii) {
    ...
    for (unsigned n = 0; n != FromElem.fromFieldVec_.size(); ++n) {
      ...
      const size_t sizeOfField =
        field_bytes_per_entity(*toFieldBaseField, theNode) / sizeof(double);
      ...
      if (!toField)
        throw std::runtime_error("Receiving field undefined on mesh object.");
