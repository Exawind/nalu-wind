...
public:
   ...
   *
   *  @param algType Type of algorithm being registered (e.g., INLET, WALL,
   * OPEN)
  ...
  void register_edge_algorithm(
    ...
    Args&&... args)
  {
    const std::string algName = unique_name(algType, "edge", algSuffix);

    register_algorithm_impl<EdgeAlg>(
      part, algName, std::forward<Args>(args)...);
  }
    ...
    Args&&... args)
  {
    ...
    register_algorithm_impl<Algorithm>(
      part, algName, std::forward<Args>(args)...);
  }
    ...
    Args&&... args)
  {
    ...
    const std::string entityName =
      "face_" + topo.name() + "_" + elemTopo.name();
