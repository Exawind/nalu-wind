...
template <
  typename ELEMDATAREQUESTSTYPE,
  typename TEAMTYPE = sierra::kynema-ugf::DeviceTeamHandleType,
  typename SHMEM = sierra::kynema-ugf::DeviceShmem>
inline int
  ...
  const ELEMDATAREQUESTSTYPE& elemDataNeeded)
{
  int bytes_per_thread =
    (rhsSize + lhsSize) * sizeof(double) + (2 * scratchIdsSize) * sizeof(int) +
    sierra::kynema-ugf::get_num_bytes_pre_req_data<double>(
