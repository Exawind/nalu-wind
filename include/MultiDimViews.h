...
public:
  ...
  : ordinals(get_shmem_view_1D<int,TEAMHANDLETYPE,SHMEM>(team, maxOrdinal+1)),
  ...
  {
    SharedMemView1D& vref = views_1D[ordinals(ordinal)];
    return vref;
  }
  ...
  void add_1D_view(unsigned ordinal, const SharedMemView1D& view)
  {
    ...
    ordinals(ordinal) = views_1D_size;
    ++views_1D_size;
  }
  ...
  void add_2D_view(unsigned ordinal, const SharedMemView2D& view)
  {
    ...
    ordinals(ordinal) = views_2D_size;
    ++views_2D_size;
  }
  ...
  void add_3D_view(unsigned ordinal, const SharedMemView3D& view)
  {
    ...
    ordinals(ordinal) = views_3D_size;
    ++views_3D_size;
  }
  ...
  void add_4D_view(unsigned ordinal, const SharedMemView4D& view)
  {
    ...
    ordinals(ordinal) = views_4D_size;
    ++views_4D_size;
  }

public:
  SharedMemView<int*,SHMEM> ordinals;
  SharedMemView1D views_1D[50];
  unsigned views_1D_size;
  SharedMemView2D views_2D[50];
  unsigned views_2D_size;
  SharedMemView3D views_3D[50];
  unsigned views_3D_size;
  SharedMemView4D views_4D[50];
