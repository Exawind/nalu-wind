...
OutputInfo::load(const YAML::Node& y_node)
{
  ...
  if (y_output) {
    ...
    get_if_present(
      y_output, "output_data_base_name", outputDBName_, outputDBName_);

    // catalyst file name
    get_if_present(
      y_output, "catalyst_file_name", catalystFileName_, catalystFileName_);

    // paraview script name
    get_if_present(
      y_output, "paraview_script_name", paraviewScriptName_,
      paraviewScriptName_);
    ...
    if (y_output["output_forced_wall_time"]) {
      ...
      userWallTimeResults_.second =
        y_output["output_forced_wall_time"].as<double>();
    }
    ...
    if (y_output["compression_level"]) {
      ...
      outputPropertyManager_->add(
        Ioss::Property("COMPRESSION_LEVEL", outputCompressionLevel_));
      ...
      get_if_present(
        y_output, "compression_shuffle", outputCompressionShuffle_,
        outputCompressionShuffle_);
  ...
  if (y_restart) {
    // some sort of intent to manage a restart event - either clean or restart
    // run
    hasRestartBlock_ = true;

    // restart output data base name
    get_if_present(
      y_restart, "restart_data_base_name", restartDBName_, restartDBName_);
    ...
    if (y_restart["restart_forced_wall_time"]) {
      ...
      userWallTimeRestart_.second =
        y_restart["restart_forced_wall_time"].as<double>();
    }

    // determine if we want nodeset restart output
    get_if_present(
      y_restart, "restart_node_set", restartNodeSet_, restartNodeSet_);

    // max data base size for restart
    get_if_present(
      y_restart, "max_data_base_step_size", restartMaxDataBaseStepSize_,
      restartMaxDataBaseStepSize_);

    // compression options; add to manager
    if (y_restart["compression_level"]) {
      restartCompressionLevel_ = y_restart["compression_level"].as<int>();
      restartPropertyManager_->add(
        Ioss::Property("COMPRESSION_LEVEL", restartCompressionLevel_));
      ...
      // only allow for shuffle if compression is active
      get_if_present(
        y_restart, "compression_shuffle", restartCompressionShuffle_,
        restartCompressionShuffle_);
