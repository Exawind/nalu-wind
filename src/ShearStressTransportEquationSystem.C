  ...
  const WallBoundaryConditionData& wallBCData)
{
  ...
  realm_.geometryAlgDriver_->register_wall_func_algorithm<WallFuncGeometryAlg>(
    sierra::kynema-ugf::WALL, part, get_elem_topo(realm_, *part), "sst_geometry_wall",
  ...
  stk::mesh::NgpField<double>& sdr)
{
  ...
  kynema-ugf_ngp::run_entity_algorithm(
    ...
    KOKKOS_LAMBDA(const kynema-ugf_ngp::NGPMeshTraits<>::MeshIndex& mi) {
...
ShearStressTransportEquationSystem::compute_f_one_blending()
{
  ...
  const auto& viscosity = kynema-ugf_ngp::get_ngp_field(meshInfo, "viscosity");
  const auto& dkdx =
