...
NonConformalManager::~NonConformalManager()
{
  ...
  for (ic = nonConformalInfoVec_.begin(); ic != nonConformalInfoVec_.end();
       ++ic)
    delete (*ic);
}
...
NonConformalManager::initialize()
{
  ...
  if (realm_.get_activate_memory_diagnostic()) {
    KynemaUGFEnv::self().kynema-ugfOutputP0()
      << "KynemaUGFMemory::NonConformalManager::initialize() Begin: " << std::endl;
    realm_.provide_memory_summary();
  }

  elemsToGhost_.clear();

  // loop over nonConformalInfo and initialize to update the elemsToGhost_
  // vector.
  ...
  if (nonConformalGhosting_ != NULL) {
    ...
    // We want both elemsToGhost_ to only contain elements not already ghosted,
    // and a list of receive-ghosts that no longer need to be ghosted.
    compute_precise_ghosting_lists(
      realm_.bulk_data(), elemsToGhost_, currentSendGhosts, recvGhostsToRemove);
  }
  ...
  // ensure that the coordinates for the ghosted elements (required for the fine
  ...
  if (realm_.get_activate_memory_diagnostic()) {
    KynemaUGFEnv::self().kynema-ugfOutputP0()
      << "KynemaUGFMemory::NonConformalManager::initialize() End: " << std::endl;
    realm_.provide_memory_summary();
  }
  ...
  std::vector<stk::mesh::EntityKey>& recvGhostsToRemove)
{
  ...
  if (nonConformalGhosting_ == NULL) {
    // create new ghosting
    std::string theGhostName = "kynema-ugf_nonConformal_ghosting";
    nonConformalGhosting_ = &bulk_data.create_ghosting(theGhostName);
  }

  bulk_data.change_ghosting(
    *nonConformalGhosting_, elemsToGhost_, recvGhostsToRemove);
