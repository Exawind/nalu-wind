    ...
    const YAML::Node y_systems = expect_sequence(y_equation_system, "systems");
    {
      for (size_t isystem = 0; isystem < y_systems.size(); ++isystem) {
        ...
        if (expect_map(y_system, "LowMachEOM", true)) {
          ...
          if (KynemaUGFEnv::self().debug())
            KynemaUGFEnv::self().kynema-ugfOutputP0() << "eqSys = tke/sdr " << std::endl;
          ...
          if (KynemaUGFEnv::self().debug())
            KynemaUGFEnv::self().kynema-ugfOutputP0() << "eqSys = tke/tdr " << std::endl;
          ...
          if (KynemaUGFEnv::self().debug())
            KynemaUGFEnv::self().kynema-ugfOutputP0() << "eqSys = tke " << std::endl;
          ...
          if (KynemaUGFEnv::self().debug())
            KynemaUGFEnv::self().kynema-ugfOutputP0() << "eqSys = enthalpy " << std::endl;
          double minT = 250.0;
          double maxT = 3000.0;
  ...
  const PeriodicBoundaryConditionData& periodicBCData)
{
  ...
  if (NULL == masterMeshPart) {
    KynemaUGFEnv::self().kynema-ugfOutputP0() << "Sorry, no part name found by the name "
    ...
    if (masterMeshParts.size() > 1)
      KynemaUGFEnv::self().kynema-ugfOutputP0() << "Surface has subsets active; please "
...
EquationSystems::initialize()
{
  ...
  for (EquationSystem* eqSys : equationSystemVector_) {
    ...
    double start_time_eq = KynemaUGFEnv::self().kynema-ugf_time();
    eqSys->initialize();
    double end_time_eq = KynemaUGFEnv::self().kynema-ugf_time();
