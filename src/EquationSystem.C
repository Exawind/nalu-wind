...
EquationSystem::load(const YAML::Node& node)
{
  ...
  if (realm_.query_for_overset()) {
    get_if_present_no_default(
      node, "decoupled_overset_solve", decoupledOverset_);
...
//--------------------------------------------------------------------------
void
EquationSystem::set_nodal_gradient(const std::string& dofName)
{
...
EquationSystem::dump_eq_time()
{
  ...
  double l_timer[6] = {timerAssemble_, timerLoadComplete_, timerSolve_,
                       timerMisc_,     timerInit_,         timerPrecond_};
  double g_min[6] = {};
  double g_max[6] = {};
  double g_sum[6] = {};
...
//--------------------------------------------------------------------------
void
EquationSystem::update_iteration_statistics(const int& iters)
{
  const double iterations = (double)iters;
  avgLinearIterations_ =
    (nonLinearIterationCount_ * avgLinearIterations_ + iterations) /
...
//--------------------------------------------------------------------------
void
EquationSystem::assemble_and_solve(stk::mesh::FieldBase* deltaSolution)
{
  int error = 0;
...
//--------------------------------------------------------------------------
bool
EquationSystem::bc_data_specified(const UserData& userData, std::string& name)
{
...
//--------------------------------------------------------------------------
UserDataType
EquationSystem::get_bc_data_type(const UserData& userData, std::string& name)
{
...
//--------------------------------------------------------------------------
void
EquationSystem::create_constraint_algorithm(stk::mesh::FieldBase* theField)
{
  ...
  if (itc == solverAlgDriver_->solverConstraintAlgMap_.end()) {
    ...
    if (decoupledOverset_)
      theAlg =
        new AssembleOversetDecoupledAlgorithm(realm_, nullptr, this, theField);
    else
      theAlg = new AssembleOversetSolverConstraintAlgorithm(
        realm_, nullptr, this, theField);
...
{
  bool noInvalidNamesFound =
    KernelBuilderLog::self().print_invalid_kernel_names(
  ...
  if (!noInvalidNamesFound) {
    ...
    std::string msg = "Invalid supplemental algorithms name(s) for " +
                      eqnTypeName_ + ". See log for details.";
...
void
EquationSystem::register_abltop_bc(
  stk::mesh::Part* part,
