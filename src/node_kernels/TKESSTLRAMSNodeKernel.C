...
TKESSTLRAMSNodeKernel::TKESSTLRAMSNodeKernel(
  const stk::mesh::MetaData& meta, const std::string coordsName)
  : NGPNodeKernel<TKESSTLRAMSNodeKernel>(),
...
void
TKESSTLRAMSNodeKernel::setup(Realm& realm)
{
  ...
  const stk::mesh::FastMeshIndex& node)
{
  ...
  // Blend into SST coefficient
  const DblType betaStarBlend =
  ...
  // SUST source term
  const DblType Dkamb = betaStarBlend * density * sdrAmb_ * tkeAmb_;
