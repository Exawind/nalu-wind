...
SDRSSTIDDESABLNodeKernel::SDRSSTIDDESABLNodeKernel(
  const stk::mesh::MetaData& meta
) : NGPNodeKernel<SDRSSTIDDESABLNodeKernel>(),
...
void
SDRSSTIDDESABLNodeKernel::setup(Realm& realm)
{
...
void
SDRSSTIDDESABLNodeKernel::execute(
  ...
  const stk::mesh::FastMeshIndex& node)
{
  ...
  const DblType eddyLengthRANS = stk::math::sqrt(tke) / stk::math::max(betaStar_ * sdr, small);
  ...
  const DblType Dk = density * stk::math::sqrt(tke) * tke / stk::math::max(eddyLengthDES, small);
