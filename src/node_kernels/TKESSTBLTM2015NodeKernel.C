  ...
  const stk::mesh::FastMeshIndex& node)
{
  ...
  const DblType alphaTKE = 1.0; // tke forcing relaxation factor
...
//  Pk = tvisc * sijMag * vortMag; // Pk based on Kato-Launder formulation. Recommended in Menter (2015) to avoid excessive levels of TKE in stagnation regions
  ...
  if (coords[0] < -0.04) {
//    tkeFreestream  = tke.turbKinEnergy_;
    tkeFreestream  = 0.50948376;
...
//  }
  ...
  //
  rhs(0) += (Pk + Pklim - Dk + tkeForcing) * dVol;
