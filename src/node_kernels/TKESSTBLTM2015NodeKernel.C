...
//-------------------------------------------------------------
  ...
  DblType Fonlim = stk::math::min(stk::math::max(Rev / 2.2 / Retclim - 1.0, 0.0), 3.0);

  Pk = gamint * tvisc * sijMag * vortMag; // Pk based on Kato-Launder formulation
  Pklim = 5.0 * Ck_BLT * stk::math::max(gamint - 0.2, 0.0) * (1.0 - gamint) * Fonlim * stk::math::max(3.0 * CSEP * visc - tvisc, 0.0) * sijMag * vortMag;
  const DblType Dk = betaStar_ * density * sdr * tke * stk::math::max(gamint, 0.1);
