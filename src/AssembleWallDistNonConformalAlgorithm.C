    ...
    for (auto fdgi : dgInfoVec) {
      for (size_t k = 0; k < fdgi.size(); ++k) {
        ...
        const int* c_face_node_ordinals =
          curMESCS->side_node_ordinals(curFaceOrd);
        const int* o_face_node_ordinals =
          oppMESCS->side_node_ordinals(oppFaceOrd);
        ...
        const double* c_areaVec =
          stk::mesh::field_data(*exposedAreaVec_, curFace);
        double c_amag = 0.0;
        ...
        if (useCurrentNormal_) {
          ...
          oppMEFC->general_normal(
            oppIsoParCrd.data(), ws_oppCoords.data(), oppNx.data());
        }
        ...
        curMEFC->general_shape_fcn(
          1, curIsoParCrd.data(), ws_c_gen_shpf.data());
        ...
        oppMEFC->general_shape_fcn(
          1, oppIsoParCrd.data(), ws_o_gen_shpf.data());
