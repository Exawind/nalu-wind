...
{
  bool thisEntityIsMaster = (bulk.identifier(entity) == kynema-ugfId);
  if (thisEntityIsMaster) {
    return entity;
  }
  stk::mesh::Entity master = bulk.get_entity(stk::topology::NODE_RANK, kynema-ugfId);
  if (throwIfMasterNotFound && !bulk.is_valid(master)) {
    ...
    STK_ThrowRequireMsg(
      ...
      "get_entity_master, P"
        << bulk.parallel_rank() << " failed to get entity for kynema-ugfId=" << kynema-ugfId
