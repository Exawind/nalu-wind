...
SolutionNormPostProcessing::~SolutionNormPostProcessing()
{
  ...
  for (ii = populateExactNodalFieldAlg_.begin();
       ii != populateExactNodalFieldAlg_.end(); ++ii)
    delete *ii;
}
...
//--------------------------------------------------------------------------
void
SolutionNormPostProcessing::load(const YAML::Node& y_node)
{
  // output for results
  ...
  if (y_norm) {
    // output frequency
    get_if_present(
      y_norm, "output_frequency", outputFrequency_, outputFrequency_);
...
SolutionNormPostProcessing::setup()
{
  ...
  const std::vector<std::string> targetNames =
    realm_.get_physics_target_names();
  ...
  for (size_t k = 0; k < dofFunctionVec_.size(); ++k) {
    ...
    // find the field
    const stk::mesh::FieldBase* dofField =
      metaData.get_field(stk::topology::NODE_RANK, dofName);
    if (NULL == dofField)
      throw std::runtime_error(
        "SolutionNorm::setup no dof field by the name of: " + dofName);
    ...
    for (size_t j = 0; j < partVec_.size(); ++j) {
      ...
      stk::mesh::put_field_on_mesh(
        *exactDofField, *targetPart, dofSize, nullptr);
  ...
  stk::mesh::Part* part)
{
  ...
  if (functionName == "SteadyTaylorVortexVelocity") {
    theAuxFunc = new SteadyTaylorVortexVelocityAuxFunction(
      0, realm_.meta_data().spatial_dimension());
  } else if (functionName == "VariableDensityVelocity") {
    theAuxFunc = new VariableDensityVelocityAuxFunction(
      0, realm_.meta_data().spatial_dimension());
  } else if (functionName == "VariableDensityNonIsoVelocity") {
    theAuxFunc = new VariableDensityVelocityAuxFunction(
      0, realm_.meta_data().spatial_dimension());
  } else if (functionName == "SteadyTaylorVortexGradPressure") {
    theAuxFunc = new SteadyTaylorVortexGradPressureAuxFunction(
      0, realm_.meta_data().spatial_dimension());
    ...
    theAuxFunc = new BoussinesqNonIsoVelocityAuxFunction(
      0, realm_.meta_data().spatial_dimension());
    ...
    theAuxFunc = new KovasznayVelocityAuxFunction(
      0, realm_.meta_data().spatial_dimension());
  } else if (functionName == "kovasznay_dpdx") {
    theAuxFunc = new KovasznayPressureGradientAuxFunction(
      0, realm_.meta_data().spatial_dimension());
  } else if (functionName == "convecting_taylor_vortex") {
    theAuxFunc = new ConvectingTaylorVortexVelocityAuxFunction(
      0, realm_.meta_data().spatial_dimension());
  } else if (functionName == "convecting_taylor_vortex_dpdx") {
    theAuxFunc = new ConvectingTaylorVortexPressureGradAuxFunction(
      0, realm_.meta_data().spatial_dimension());
  } else if (functionName == "OneTwoTenVelocity") {
    theAuxFunc = new OneTwoTenVelocityAuxFunction(
      0, realm_.meta_data().spatial_dimension());
  } else if (functionName == "wind_energy_taylor_vortex") {
    theAuxFunc = new WindEnergyTaylorVortexAuxFunction(
      0, realm_.meta_data().spatial_dimension(), std::vector<double>());
  } else if (functionName == "wind_energy_taylor_vortex_dpdx") {
    theAuxFunc = new WindEnergyTaylorVortexPressureGradAuxFunction(
      0, realm_.meta_data().spatial_dimension(), std::vector<double>());
  } else {
    throw std::runtime_error(
  ...
  AuxFunctionAlgorithm* auxAlg = new AuxFunctionAlgorithm(
    realm_, part, exactDofField, theAuxFunc, stk::topology::NODE_RANK);
...
{
  // check for proper count; return if not an output count (worry about field
  // output sync?)
  ...
  stk::mesh::Selector s_locall_owned = metaData.locally_owned_part() &
  ...
  stk::mesh::BucketVector const& node_buckets =
    bulkData.get_buckets(stk::topology::NODE_RANK, s_locall_owned);
  for (stk::mesh::BucketVector::const_iterator ib = node_buckets.begin();
    ...
    for (size_t j = 0; j < fieldPairVec_.size(); ++j) {
      ...
      // extract fields
      const double* dofField =
        (double*)stk::mesh::field_data(*(fieldPairVec_[j].first), b);
      double* exactDofField =
        (double*)stk::mesh::field_data(*(fieldPairVec_[j].second), b);
      ...
      for (stk::mesh::Bucket::size_type k = 0; k < length; ++k) {
        ...
        for (int i = 0; i < fieldSize; ++i) {
          const double diff = std::abs(
            dofField[k * fieldSize + i] - exactDofField[k * fieldSize + i]);
          // norms...
  ...
  if (NaluEnv::self().parallel_rank() == 0) {
    ...
    for (size_t j = 0; j < fieldPairVec_.size(); ++j) {
      ...
      for (int i = 0; i < fieldSize; ++i) {
        myfile << std::setprecision(percision_) << std::setw(w_) << dofName
               << "[" << i << "]" << std::setw(w_) << timeStepCount
               << std::setw(w_) << currentTime << std::setw(w_) << g_nodeCount
               << std::setw(w_) << g_LooNorm[offSet + i] << std::setw(w_)
               << g_L12Norm[offSet + i] / g_nodeCount << std::setw(w_)
               << std::sqrt(
                    g_L12Norm[offSet + i + totalDofCompSize_] / g_nodeCount)
               << std::setw(w_) << std::endl;
      }
      // increment offset
      offSet += fieldSize;
    }
