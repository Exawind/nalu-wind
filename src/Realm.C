...
Realm::create_output_mesh()
{
  ...
  if (!outputManager_->infoVec_.empty()) {

    double start_time = NaluEnv::self().nalu_time();
    NaluEnv::self().naluOutputP0() << "Realm::create_output_mesh(): Begin" << std::endl;

    outputManager_->create_output_mesh(ioBroker_);
...
Realm::create_restart_mesh()
{
  ...
  if (outputManager_->has_restart_output()) {
    ...
    ioBroker_->get_output_io_region(restartFileIndex_)->get_database()->set_cycle_count(outputInfo.restartMaxDataBaseStepSize_);
  }
...
Realm::provide_output()
{
  ...
  outputManager_->perform_outputs(
    timeStepCount, currentTime, ioBroker_, wallTimeStart_);
  if (doPromotion_) {
