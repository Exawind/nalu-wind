...
struct sortIntLowHigh
{
  sortIntLowHigh() {}
  bool operator()(
    const std::pair<theKey, theKey>& p1, const std::pair<theKey, theKey>& p2)
  {
...
//-------- destructor ------------------------------------------------------
//--------------------------------------------------------------------------
NonConformalInfo::~NonConformalInfo() { delete_dgInfo(); }
...
NonConformalInfo::construct_dgInfo()
{
  ...
  stk::mesh::Selector s_locally_owned_union =
    meta_data.locally_owned_part() & stk::mesh::selectUnion(currentPartVec_);
  ...
  for (stk::mesh::BucketVector::const_iterator ib = face_buckets.begin();
    ...
    for (stk::mesh::Bucket::size_type k = 0; k < length; ++k) {
      ...
      const stk::mesh::ConnectivityOrdinal* face_elem_ords =
        bulk_data.begin_element_ordinals(face);
...
NonConformalInfo::construct_bounding_points()
{
  ...
  for (ii = dgInfoVec_.begin(); ii != dgInfoVec_.end(); ++ii) {
    ...
    stk::mesh::Entity const* face_node_rels =
      bulk_data.begin_nodes(firstDgInfo->currentFace_);
    ...
    for (size_t k = 0; k < theVec.size(); ++k) {
      ...
      const double* intgLoc = useShifted ? meFC->integration_location_shift()
                                         : meFC->integration_locations();
      ...
      // save face iso-parametric coordinates; extract conversion factor from
      // CVFEM to isInElement
      const double conversionFac = meFC->scal_to_standard_iso_factor();
      for (int j = 0; j < nDim - 1; ++j) {
        dgInfo->currentIsoParCoords_[j] =
          conversionFac * intgLoc[currentFaceIp * (nDim - 1) + j];
      }
...
void
NonConformalInfo::delete_range_points_found(
  ...
  const std::vector<std::pair<theKey, theKey>>& searchKeyPair) const
{
  ...
  std::vector<boundingSphere> difference(SphereVec.size());
  {
    const auto it = std::set_difference(
      SphereVec.begin(), SphereVec.end(), keys_found.begin(), keys_found.end(),
...
void
NonConformalInfo::repeat_search_if_needed(
  ...
  for (ii = searchKeyPair_.begin(); ii != searchKeyPair_.end(); ++ii) {
    ...
    if ((box_proc == theRank) && (pt_proc != theRank)) {
      ...
      stk::mesh::Entity face =
        bulk_data.get_entity(meta_data.side_rank(), theBox);
...
NonConformalInfo::complete_search()
{
  ...
  for (ii = dgInfoVec_.begin(); ii != dgInfoVec_.end(); ++ii) {
    ...
    for (size_t k = 0; k < theVec.size(); ++k) {
      ...
      std::pair<
        ...
        std::vector<std::pair<theKey, theKey>>::const_iterator>
        p2 = std::equal_range(
          searchKeyPair_.begin(), searchKeyPair_.end(), localGaussPointId,
          compareGaussPoint());
      ...
      if (p2.first == p2.second) {
        ...
        for (std::vector<std::pair<theKey, theKey>>::const_iterator jj =
          ...
          if (theRank == pt_proc) {
            ...
            // yes, I own the point... However, what about the face element? Who
            // owns that?

            // proceed as required; all elements should have already been
            // ghosted via the coarse search
            stk::mesh::Entity opposingFace =
              bulk_data.get_entity(meta_data.side_rank(), theBox);
            if (!(bulk_data.is_valid(opposingFace)))
              throw std::runtime_error("no valid entry for face element");

            int opposingFaceIsGhosted =
              bulk_data.bucket(opposingFace).owned() ? 0 : 1;
            ...
            dgInfo->allOpposingFaceIds_.push_back(
              bulk_data.identifier(opposingFace));

            // find distance between true current gauss point coords (the point)
            // and the candidate bounding box
            const double nearDistance = meFC->isInElement(
              &theElementCoords[0], &(currentGaussPointCoords[0]),
            ...
            if (nearDistance < dgInfo->bestX_) {
              ...
              if (dynamicSearchTolAlg_) {
                // find the projected normal distance between point and
                // centroid; all we need is an approximation
                meFC->interpolatePoint(
                  nDim, &opposingIsoParCoords[0], &theElementCoords[0],
                  &bestElemIpCoords[0]);
                ...
                // If the nearest distance between the surfaces at this point is
                // smaller then the current distance can be reduced a bit.
                // Otherwise make sure the current distance is increased as
                // needed.
                if (nearestDistance < dgInfo->nearestDistance_) {
                  const double relax = 0.8;
                  dgInfo->nearestDistance_ = relax * nearestDistanceSaved +
              ...
              const stk::mesh::ConnectivityOrdinal* face_elem_ords =
                bulk_data.begin_element_ordinals(opposingFace);
  ...
  // check for reuse and also provide diagnostics on sizes for opposing surface
  // set
  ...
  for (size_t iv = 0; iv < dgInfoVec_.size(); ++iv) {
    ...
    for (size_t k = 0; k < theVec.size(); ++k) {
      ...
      auto itF = std::find(
        dgInfo->allOpposingFaceIdsOld_.begin(),
...
NonConformalInfo::construct_bounding_boxes()
{
  ...
  stk::mesh::Selector s_locally_owned_union =
    meta_data.locally_owned_part() & stk::mesh::selectUnion(opposingPartVec_);
  ...
  for (ii = dgInfoVec_.begin(); ii != dgInfoVec_.end(); ++ii) {
    ...
    for (size_t k = 0; k < theVec.size(); ++k) {
      ...
      meFCCurrent->interpolatePoint(
        nDim, &currentIsoParCoords[0], &currentFaceNodalCoords[0],
      ...
      stk::mesh::Entity const* opposing_face_node_rels =
        bulk_data.begin_nodes(theBestFace);
      ...
      meFCOpposing->interpolatePoint(
        nDim, &opposingIsoParCoords[0], &opposingFaceNodalCoords[0],
      ...
      for (int j = 0; j < nDim; ++j)
        distanceNorm += std::pow(
          checkCurrentFaceGaussPointCoords[j] -
