      ...
      for (size_t k = 0; k < faceDgInfoVec.size(); ++k) {
        ...
        for (int ic = 0; ic < currentNodesPerElement; ++ic) {
          ...
          for (int j = 0; j < nDim; ++j) {
            ...
            const double divUstress = 2.0 / 3.0 * currentDiffFluxCoeffBip *
                                      dndxj * uxj * nxj * includeDivU_;
            ...
            for (int i = 0; i < nDim; ++i) {
              ...
              currentDiffFluxBip[i] +=
                -currentDiffFluxCoeffBip * dndxj * nxj * uxi + divUstress;

              // -mu*duj/dxi*Aj
              currentDiffFluxBip[i] +=
                -currentDiffFluxCoeffBip * dndxi * nxj * uxj;
            }
          }
        }
        ...
        for (int ic = 0; ic < opposingNodesPerElement; ++ic) {
          ...
          for (int j = 0; j < nDim; ++j) {
            ...
            const double divUstress = 2.0 / 3.0 * opposingDiffFluxCoeffBip *
                                      dndxj * uxj * nxj * includeDivU_;
            ...
            for (int i = 0; i < nDim; ++i) {
              ...
              opposingDiffFluxBip[i] +=
                -opposingDiffFluxCoeffBip * dndxj * nxj * uxi + divUstress;

              // -mu*duj/dxi*Aj
              opposingDiffFluxBip[i] +=
                -opposingDiffFluxCoeffBip * dndxi * nxj * uxj;
            }
          }
        }
        ...
        // compute general shape function at current and opposing integration
        // points
        ...
        const double penaltyIp =
          (currentDiffFluxCoeffBip * currentInverseLength +
           opposingDiffFluxCoeffBip * opposingInverseLength) /
          2.0;

        for (int i = 0; i < nDim; ++i) {

          // non conformal diffusive flux
          const double ncDiffFlux =
          ...
          p_rhs[indexR] -=
            ((ncDiffFlux + penaltyIp * (currentUBip[i] - opposingUBip[i])) *
               c_amag +
             ncAdv);
          ...
          const double lhsFacC =
            penaltyIp * c_amag + (eta_ * abs_tmdot + tmdot) / 2.0;
          ...
          for (int ic = 0; ic < currentNodesPerElement; ++ic) {
            ...
            for (int j = 0; j < nDim; ++j) {
              ...
              p_lhs[rowR + icNdim + i] +=
                -currentDiffFluxCoeffBip * dndxj * nxj * c_amag / 2.0;
              // -mu*duj/dxi*nj*dS
              p_lhs[rowR + icNdim + j] +=
                -currentDiffFluxCoeffBip * dndxi * nxj * c_amag / 2.0;
            }
          }

          // sensitivities; opposing face (penalty and advection); use general
          // shape function for this single ip
          const double lhsFacO =
            penaltyIp * c_amag + (eta_ * abs_tmdot - tmdot) / 2.0;
          for (int ic = 0; ic < opposingNodesPerFace; ++ic) {
            const int icNdim =
              (o_face_node_ordinals[ic] + currentNodesPerElement) * nDim;
          ...
          for (int ic = 0; ic < opposingNodesPerElement; ++ic) {
            ...
            for (int j = 0; j < nDim; ++j) {
              ...
              // -mu*dui/dxj*nj*dS (divU neglected)
              p_lhs[rowR + icNdim + i] -=
                -opposingDiffFluxCoeffBip * dndxj * nxj * c_amag / 2.0;
              // -mu*duj/dxi*nj*dS
              p_lhs[rowR + icNdim + j] -=
                -opposingDiffFluxCoeffBip * dndxi * nxj * c_amag / 2.0;
            }
          }
        }
