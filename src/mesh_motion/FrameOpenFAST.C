...
void
FrameOpenFAST::update_coordinates_velocity(const double time)
{
  ...
  if (fsiTurbineData_ != NULL) {
    ...
    // displacement->field_of_state(stk::mesh::StateN); VectorFieldType
    // &displacementNp1 = displacement->field_of_state(stk::mesh::StateNP1);
    VectorFieldType* meshVelocity = meta_.get_field<VectorFieldType>(
      stk::topology::NODE_RANK, "mesh_velocity");

    // get the parts in the current motion frame
    stk::mesh::Selector sel =
      stk::mesh::selectUnion(fsiTurbineData_->getPartVec());
