...
TurbineSurrogateKernel::TurbineSurrogateKernel(
  stk::mesh::MetaData& meta, const YAML::Node& node)
  : NgpMotionKernel<TurbineSurrogateKernel>()
{
  load(node);
...
void
TurbineSurrogateKernel::load(const YAML::Node& node)
{
  ...
  if (node["axis"]) {
    ...
    NaluEnv::self().naluOutputP0()
      << "TurbineSurrogateKernel: axis of rotation not supplied; will use 0,0,1"
      << std::endl;
...
KOKKOS_FUNCTION
mm::TransMatType
TurbineSurrogateKernel::build_transformation(
...
KOKKOS_FUNCTION
mm::ThreeDVecType
TurbineSurrogateKernel::compute_velocity(
  const double& time,
