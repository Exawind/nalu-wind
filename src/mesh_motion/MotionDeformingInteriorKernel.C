...
void MotionDeformingInteriorKernel::load(const YAML::Node& node)
{
  ...
  if( !node["xyz_min"] )
    NaluEnv::self().naluOutputP0() << "MotionDeformingInteriorKernel: Need to define lower bounds of mesh that deform" << std::endl;
  for (int d=0; d < nalu_ngp::NDimMax; ++d)
    xyzMin_[d] = node["xyz_min"][d].as<double>();

  // get lower bounds of deforming part of mesh
  if( !node["xyz_max"] )
    NaluEnv::self().naluOutputP0() << "MotionDeformingInteriorKernel: Need to define upper bounds of mesh that deform" << std::endl;
  for (int d=0; d < nalu_ngp::NDimMax; ++d)
...
mm::TransMatType MotionDeformingInteriorKernel::build_transformation(
  const double& time,
  const mm::ThreeDVecType& xyz)
{
  ...
  for (int d=0; d < nalu_ngp::NDimMax; d++)
  {
    ...
    curr_radius[d] = radius[d] + amplitude_[d]*(1 - stk::math::cos(2*M_PI*frequency_[d]*motionTime));

    scaling[d] = curr_radius[d]/radius[d];
    if(radius[d] <= DBL_EPSILON) scaling[d] = 1.0;
...
mm::ThreeDVecType MotionDeformingInteriorKernel::compute_velocity(
  const double& time,
  ...
  const mm::ThreeDVecType& /* cxyz */)
{
  ...
  for (int d=0; d < nalu_ngp::NDimMax; d++)
  {
    ...
    osclVelocity[d] =
      amplitude_[d] * stk::math::sin(2*M_PI*frequency_[d]*time) * 2*M_PI*frequency_[d] / radius[d];
    if(radius[d] <= DBL_EPSILON) osclVelocity[d] = 0.0;
