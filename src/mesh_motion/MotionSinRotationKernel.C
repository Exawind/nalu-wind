...
MotionSinRotationKernel::MotionSinRotationKernel(const YAML::Node& node)
  : NgpMotionKernel<MotionSinRotationKernel>()
{
  load(node);
}
...
void MotionSinRotationKernel::load(const YAML::Node& node)
{
  ...
  if( node["axis"] ) {
    ...
    NaluEnv::self().naluOutputP0() << "MotionSinRotationKernel: axis of rotation not supplied; will use 0,0,1" << std::endl;
...
mm::TransMatType MotionSinRotationKernel::build_transformation(
  const double& time,
  const mm::ThreeDVecType& /* xyz */)
{
  ...
  angle_ = amplt_ * stk::math::sin(omega_ * (motionTime-startTime_) );
  double angle = angle_;
...
mm::ThreeDVecType MotionSinRotationKernel::compute_velocity(
  const double& time,
  ...
  const mm::ThreeDVecType& cxyz)
{
  ...
  ang_vel_ = amplt_ * omega_ * stk::math::cos(omega_ * (motionTime-startTime_) );
  mm::ThreeDVecType relCoord;
  mm::ThreeDVecType vecOmega;
  for (int d=0; d < nalu_ngp::NDimMax; d++) {
    relCoord[d] = cxyz[d] - transOrigin[d];
    vecOmega[d] = ang_vel_*unitVec[d];
  }
