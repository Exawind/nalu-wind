  ...
  if (waveModel_ == 1) {
    ...
    curr_disp[2] =
      ...
      (eps_ * stk::math::cos(phase)                            // first order term
       ...
       + stk::math::pow(eps_, 3) * b31_ * (stk::math::cos(phase) - stk::math::cos(3. * phase)) +
...
MotionWavesKernel::Stokes_coefficients()
{
  ...
  double CTh =
    (1 + stk::math::exp(-2. * kd)) / (1 - stk::math::exp(-2 * kd)); // Hyperbolic cotangent
  ...
  c2_ = stk::math::sqrt(Th) * (2 + 7 * stk::math::pow(S, 2)) / (4 * stk::math::pow(1 - S, 2));
  d2_ = -stk::math::sqrt(CTh) / 2.;
  e2_ = Th * (2 + 2 * S + 5 * stk::math::pow(S, 2)) / (4 * stk::math::pow(1 - S, 2));
  if (StokesOrder_ == 2)
    return;
  ...
  a33_ =
    (-2 * stk::math::pow(S, 2) + 11 * stk::math::pow(S, 3)) / (8 * Sh * stk::math::pow(1 - S, 3));
  b31_ = -3 * (1 + 3 * S + 3 * stk::math::pow(S, 2) + 2 * stk::math::pow(S, 3)) /
  ...
  a44_ = (10 * stk::math::pow(S, 3) - 174 * stk::math::pow(S, 4) + 291 * stk::math::pow(S, 5) +
          278 * stk::math::pow(S, 6)) /
  ...
  d4_ = stk::math::sqrt(CTh) * (2 + 4 * S + stk::math::pow(S, 2) + 2 * stk::math::pow(S, 3)) /
        (8 * stk::math::pow(1 - S, 3));
  e4_ = Th *
  ...
  a51_ = (-1184 + 32 * S + 13232 * stk::math::pow(S, 2) + 21712 * stk::math::pow(S, 3) +
          20940 * stk::math::pow(S, 4) + 12554 * stk::math::pow(S, 5) -
          500 * stk::math::pow(S, 6) - 3341 * stk::math::pow(S, 7) - 670 * stk::math::pow(S, 8)) /
         (64 * Sh * (3 + 2 * S) * (4 + S) * stk::math::pow(1 - S, 6));
  a53_ =
    (4 * S + 105 * pow(S, 2) + 198 * stk::math::pow(S, 3) - 1376 * stk::math::pow(S, 4) -
     1302 * stk::math::pow(S, 5) - 117 * stk::math::pow(S, 6) + 58 * stk::math::pow(S, 7)) /
    (32 * Sh * (3 + 2 * S) * stk::math::pow(1 - S, 6));
  a55_ = (-6 * stk::math::pow(S, 3) + 272 * stk::math::pow(S, 4) - 1552 * stk::math::pow(S, 5) +
          852 * stk::math::pow(S, 6) + 2029 * stk::math::pow(S, 7) + 430 * stk::math::pow(S, 8)) /
         (64 * Sh * (3 + 2 * S) * (4 + S) * stk::math::pow(1 - S, 6));
  b53_ = 9 *
         (132 + 17 * S - 2216 * stk::math::pow(S, 2) - 5897 * stk::math::pow(S, 3) -
          6292 * stk::math::pow(S, 4) - 2687 * stk::math::pow(S, 5) + 194 * stk::math::pow(S, 6) +
          467 * stk::math::pow(S, 7) + 82 * stk::math::pow(S, 8)) /
         (128 * (3 + 2 * S) * (4 + S) * stk::math::pow(1 - S, 6));
  b55_ = 5 *
         (300 + 1579 * S + 3176 * stk::math::pow(S, 2) + 2949 * stk::math::pow(S, 3) +
          1188 * stk::math::pow(S, 4) + 675 * stk::math::pow(S, 5) + 1326 * stk::math::pow(S, 6) +
...
MotionWavesKernel::Stokes_parameters()
{
  ...
  c_ = (c0_ + stk::math::pow(eps_, 2) * c2_ + stk::math::pow(eps_, 4) * c4_) *
       stk::math::sqrt(g_ / k_);
  Q_ = c_ * waterdepth_ * stk::math::sqrt(stk::math::pow(k_, 3) / g_) +
...
MotionWavesKernel::my_cosh_cos(int i, int j, double phase)
{
  ...
  return stk::math::pow(eps_, i) * D * j * k_ * stk::math::cosh(j * k_ * waterdepth_) *
         stk::math::cos(j * phase);
}
...
MotionWavesKernel::my_sinh_sin(int i, int j, double phase)
{
  ...
  return stk::math::pow(eps_, i) * D * j * k_ * stk::math::sinh(j * k_ * waterdepth_) *
         stk::math::sin(j * phase);
}
