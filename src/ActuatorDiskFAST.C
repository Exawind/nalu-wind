...
void ActuatorDiskFAST::execute_class_specific(
  ...
  for(std::size_t i = 0; i < numFastPoints_; i ++){
    ...
    if(infoObject->nodeType_==fast::BLADE){
      ...
      std::vector<double>& avgVec = averageForcesMap_.at(turbineNum)[radialIndex];
      for (int j=0; j<3; j++){
  ...
  for (auto&& iterPoint : actuatorPointInfoMap_) {
    ...
    case fast::BLADE:
      spread_actuator_force_to_node_vec(
        nDim,
        nodeVec,
        averageForcesMap_.at(iTurbGlob)[pointRadiusMap_.at(np)],
        &(infoObject->centroidCoords_[0]),
        *coordinates,
        *actuator_source,
        *dualNodalVolume,
...
Point ActuatorDiskFAST::get_blade_point_location(int turbNum, int bNum, int radIndex){
  std::vector<double> temp(3,0);
  const int numPntsBlade = FAST.get_numForcePtsBlade(turbNum);
  FAST.getForceNodeCoordinates(temp, radIndex+bNum*numPntsBlade+1, turbNum);
  Point answer ={temp[0],temp[1], temp[2]};
  return answer;
}

void ActuatorDiskFAST::add_swept_points_to_map(){
  ...
  for(int iTurb =0; iTurb<numTurbGlob; iTurb++){
    if(FAST.get_procNo(iTurb)==myProcId){
      ...
      // loop over each radial location and insert the correct number of points for that radius
      // this could be made to vary radially later on
      for(int i=0; i< numPntsBlade; i++){
        for( int j =0; j<numBlades; j++){
          locator.update_point_location(j, get_blade_point_location(iTurb, j, i));
        }
          ...
          for( int j=0; j<mySwept; j++){
            ...
            actuatorPointInfoMap_.insert(std::make_pair(np, make_unique<ActuatorFASTPointInfo>
              (
                ...
                searchRadius, actuatorInfo->epsilon_, // TODO(psakiev)::scale epsilon based on the arc length ratio with the tip
                fast::BLADE,
                i
              ))
            );
...
void ActuatorDiskFAST::create_point_info_map_class_specific(){
  ...
  for(int i=0; i<nGTurb; i++){
    if(myProcId == FAST.get_procNo(i)){
      ...
      averageForcesMap_.insert(std::make_pair(i,std::vector<std::vector<double>>(numPntsBlade, dummyToInsert)));
    }
  }
  ...
  if( !FAST.isDryRun() ){
    ...
    for (std::size_t i=0; i<numFastPoints_; i++){
      ...
      if(fastPoint->nodeType_==fast::BLADE && turbProcId == myProcId){
        ...
        int nForcePointsBlade = FAST.get_numForcePtsBlade(static_cast<int>(iTurb));
