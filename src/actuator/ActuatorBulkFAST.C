  ...
  epsilonOpt_("epsilonOptimal", actMeta.numberOfActuators_)
{
  ...
  // TODO(psakiev) copy functionality of ActuatorFAST::setup can do in meta/parse
  ...
  for(int i=1; i<numTurbs; i++){
    turbIdOffset_.h_view(i) = turbIdOffset_.h_view(i-1) + actMeta.numPointsTurbine_.h_view(i-1);
  }
    ...
    if(openFast_.get_procNo(iTurb)==NaluEnv::self().parallel_rank()){
      ...
      for(int np=0; np<numForcePts; np++){
          ...
          searchRadius_.h_view(iTurb) =
            std::max(epsilonLocal[0], std::max(epsilonLocal[1], epsilonLocal[2]))
              * sqrt(log(1.0/0.001));
      }
    } else {
