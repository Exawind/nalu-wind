...
{
  using SimdDataType = kynema-ugf_ngp::FaceElemSimdData<stk::mesh::NgpMesh>;
  ...
  const auto areaOps =
    kynema-ugf_ngp::simd_face_elem_nodal_field_updater(ngpMesh, warea);
  const auto distOps =
    kynema-ugf_ngp::simd_face_elem_nodal_field_updater(ngpMesh, wdist);
  const auto dBipOps =
    kynema-ugf_ngp::simd_face_elem_field_updater(ngpMesh, wdistBip);
  ...
  kynema-ugf_ngp::run_face_elem_algorithm(
