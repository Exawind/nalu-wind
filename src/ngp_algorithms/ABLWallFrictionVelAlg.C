...
ABLWallFrictionVelAlg<BcAlgTraits>::execute()
{
  ...
  const auto utauOps = kynema-ugf_ngp::simd_elem_field_updater(ngpMesh, ngpUtau);
  ...
  kynema-ugf_ngp::run_elem_par_reduce(
    algName, meshInfo, realm_.meta_data().side_rank(), faceData_, sel,
    KOKKOS_LAMBDA(ElemSimdData & edata, kynema-ugf_ngp::ArraySimdDouble2 & uSum) {
      // Unit normal vector
