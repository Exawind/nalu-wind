...
namespace {
inline void simd_add(const DoubleType& inVal, double& outVal)
{
...
MdotAlgDriver::MdotAlgDriver(
  Realm& realm,
  const bool elemContinuityEqs
) : NgpAlgDriver(realm),
    elemContinuityEqs_(elemContinuityEqs)
{}

void MdotAlgDriver::add_density_accumulation(const DoubleType& rhoAcc)
{
  simd_add(rhoAcc, rhoAccum_);
}

void MdotAlgDriver::add_inflow_mdot(const DoubleType& inflow)
{
  simd_add(inflow, mdotInflow_);
}

void MdotAlgDriver::add_open_mdot(const DoubleType& outflow)
{
  simd_add(outflow, mdotOpen_);
}

void MdotAlgDriver::add_density_accumulation(const double& rhoAcc)
{
  rhoAccum_ += rhoAcc;
}

void MdotAlgDriver::add_inflow_mdot(const double& inflow)
{
  mdotInflow_ += inflow;
}

void MdotAlgDriver::add_open_mdot(const double& outflow)
{
  mdotOpen_ += outflow;
}

void MdotAlgDriver::add_open_mdot_post(const double& outflow)
{
  mdotOpenPost_ += outflow;
}

void MdotAlgDriver::pre_work()
{
  ...
  if (isInit_ && hasOpenBC_) {
    ...
    unsigned numIp = 0;
    const auto& bkts = realm_.bulk_data().get_buckets(realm_.meta_data().side_rank(), sel);
    if (elemContinuityEqs_) {
      for (const auto* b: bkts) {
        auto* meFC = MasterElementRepo::get_surface_master_element(b->topology());
...
void MdotAlgDriver::post_work()
{
  ...
  if (realm_.solutionOptions_->activateOpenMdotCorrection_ && hasOpenBC_) {
    mdotOpenCorrection_ = (rhoAccum_ + mdotInflow_ + mdotOpen_) / mdotOpenIpCount_;
...
void MdotAlgDriver::provide_output()
{
  ...
  if (realm_.solutionOptions_->activateOpenMdotCorrection_) {
    NaluEnv::self().naluOutputP0()
      ...
      << " occurred on: " << mdotOpenIpCount_
      << " boundary integration points." << std::endl
      << "Post-corrected integrated open: "
      << std::setprecision(16) << mdotOpenPost_ << std::endl;
  }
}

void MdotAlgDriver::register_open_mdot_corrector_alg(
