...
{
  using Traits =
    kynema-ugf_ngp::NGPMeshTraits<typename Realm::NgpMeshInfo::NgpMeshType>;
  ...
  kynema-ugf_ngp::MinMaxSumScalar<double> volStats;
  kynema-ugf_ngp::MinMaxSum<double> volReducer(volStats);
...
GeometryAlgDriver::mesh_motion_prework()
{
  ...
  auto ngpFaceVelMag =
    kynema-ugf_ngp::get_ngp_field(realm_.mesh_info(), fvmFieldName, entityRank);
  ...
  auto ngpSweptVol =
    kynema-ugf_ngp::get_ngp_field(realm_.mesh_info(), svFieldName, entityRank);
  ...
  if (realm_.realmUsesEdges_) {
    ...
    auto ngpSweptVolEdgeN = kynema-ugf_ngp::get_ngp_field(
...
GeometryAlgDriver::post_work()
{
  ...
  auto& ngpDualVol = kynema-ugf_ngp::get_ngp_field(meshInfo, "dual_nodal_volume");
  ...
  if (realm_.realmUsesEdges_) {
    auto& ngpEdgeArea = kynema-ugf_ngp::get_ngp_field(
      meshInfo, "edge_area_vector", stk::topology::EDGE_RANK);
    fields.push_back(&ngpEdgeArea);

    if (realm_.has_mesh_deformation()) {
      auto& ngpedgeFaceVel =
        kynema-ugf_ngp::get_ngp_field(meshInfo, "edge_face_velocity_mag", entityRank);
      auto& ngpedgeSweptVol =
        kynema-ugf_ngp::get_ngp_field(meshInfo, "edge_swept_face_volume", entityRank);
  ...
  if (hasWallFunc_) {
    auto& wallAreaF =
      kynema-ugf_ngp::get_ngp_field(meshInfo, "assembled_wall_area_wf");
    auto& wallDistF =
      kynema-ugf_ngp::get_ngp_field(meshInfo, "assembled_wall_normal_distance");
  ...
  if (hasWallFunc_) {
    ...
    auto wdist =
      kynema-ugf_ngp::get_ngp_field(meshInfo, "assembled_wall_normal_distance");
    auto warea = kynema-ugf_ngp::get_ngp_field(meshInfo, "assembled_wall_area_wf");
