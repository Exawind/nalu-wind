...
SSTMaxLengthScaleAlg<AlgTraits>::SSTMaxLengthScaleAlg(
  ...
  stk::mesh::Part* part
    ...
    maxLengthScale_(get_field_ordinal(realm_.meta_data(), "sst_max_length_scale")),
...
void SSTMaxLengthScaleAlg<AlgTraits>::execute()
{
  ...
  const std::string algName = "compute_sst_max_length_scale_" + std::to_string(AlgTraits::topo_);
  nalu_ngp::run_elem_algorithm(
    ...
    KOKKOS_LAMBDA(const ElemInfoType& einfo) {
      ...
      const auto nodes = ngpMesh.get_nodes(stk::topology::ELEM_RANK, ngpMesh.fast_mesh_index(entity));
      ...
      for (int ip = 0; ip < AlgTraits::numScsIp_; ++ip) {
        ...
        if (maxLengthL < dx) Kokkos::atomic_add(&maxLengthL, dx - maxLengthL);
        if (maxLengthR < dx) Kokkos::atomic_add(&maxLengthR, dx - maxLengthR);
      }
    });
