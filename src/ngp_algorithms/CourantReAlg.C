...
{
  using ElemSimdDataType = kynema-ugf_ngp::ElemSimdData<stk::mesh::NgpMesh>;
  ...
  const auto cflOps = kynema-ugf_ngp::simd_elem_field_updater(ngpMesh, ngpCFL);
  const auto reyOps = kynema-ugf_ngp::simd_elem_field_updater(ngpMesh, ngpRe);
