...
SSTAMSAveragesAlg::execute()
{
  ...
  kynema-ugf_ngp::run_entity_algorithm(
    ...
    KOKKOS_LAMBDA(const Traits::MeshIndex& mi) {
      ...
      for (int i = 0; i < kynema-ugf_ngp::NDimMax; ++i)
        avgVel.get(mi, i) =
      ...
      for (int i = 0; i < kynema-ugf_ngp::NDimMax; ++i) {
        for (int j = 0; j < kynema-ugf_ngp::NDimMax; ++j) {
          avgDudx.get(mi, i * kynema-ugf_ngp::NDimMax + j) =
            weightAvg * avgDudxN.get(mi, i * kynema-ugf_ngp::NDimMax + j) +
            weightInst * dudx.get(mi, i * kynema-ugf_ngp::NDimMax + j);
        }
      }

      // Production averaging
      DblType tij[kynema-ugf_ngp::NDimMax][kynema-ugf_ngp::NDimMax];
      for (int i = 0; i < kynema-ugf_ngp::NDimMax; ++i) {
        for (int j = 0; j < kynema-ugf_ngp::NDimMax; ++j) {
          const DblType avgSij =
            0.5 * (avgDudx.get(mi, i * kynema-ugf_ngp::NDimMax + j) +
                   avgDudx.get(mi, j * kynema-ugf_ngp::NDimMax + i));
          tij[i][j] = 2.0 * alpha * (2.0 - alpha) * tvisc.get(mi, 0) * avgSij;
        }
      }

      DblType Pij[kynema-ugf_ngp::NDimMax][kynema-ugf_ngp::NDimMax];
      for (int i = 0; i < kynema-ugf_ngp::NDimMax; ++i) {
        for (int j = 0; j < kynema-ugf_ngp::NDimMax; ++j) {
          Pij[i][j] = 0.0;
          for (int m = 0; m < kynema-ugf_ngp::NDimMax; ++m) {
            Pij[i][j] +=
              avgDudx.get(mi, i * kynema-ugf_ngp::NDimMax + m) * tij[j][m] +
              avgDudx.get(mi, j * kynema-ugf_ngp::NDimMax + m) * tij[i][m];
          }
          Pij[i][j] *= 0.5;
        }
      }
      ...
      // get Mij field_data
      DblType p_Mij[kynema-ugf_ngp::NDimMax][kynema-ugf_ngp::NDimMax];
      DblType PM[kynema-ugf_ngp::NDimMax][kynema-ugf_ngp::NDimMax];
      DblType Q[kynema-ugf_ngp::NDimMax][kynema-ugf_ngp::NDimMax];
      DblType D[kynema-ugf_ngp::NDimMax][kynema-ugf_ngp::NDimMax];

      for (int i = 0; i < kynema-ugf_ngp::NDimMax; i++) {
        const int iNdim = i * kynema-ugf_ngp::NDimMax;
        for (int j = 0; j < kynema-ugf_ngp::NDimMax; j++) {
      ...
      // initialize M43 to 0
      DblType M43[kynema-ugf_ngp::NDimMax][kynema-ugf_ngp::NDimMax];
      for (int i = 0; i < kynema-ugf_ngp::NDimMax; ++i)
        for (int j = 0; j < kynema-ugf_ngp::NDimMax; ++j)
          M43[i][j] = 0.0;
      ...
      for (int l = 0; l < kynema-ugf_ngp::NDimMax; l++) {
      ...
      // zeroing out tensors
      DblType tauSGRS[kynema-ugf_ngp::NDimMax][kynema-ugf_ngp::NDimMax];
      DblType tauSGET[kynema-ugf_ngp::NDimMax][kynema-ugf_ngp::NDimMax];
      DblType tau[kynema-ugf_ngp::NDimMax][kynema-ugf_ngp::NDimMax];
      DblType Psgs[kynema-ugf_ngp::NDimMax][kynema-ugf_ngp::NDimMax];
      for (int i = 0; i < kynema-ugf_ngp::NDimMax; ++i) {
        for (int j = 0; j < kynema-ugf_ngp::NDimMax; ++j) {
          tauSGRS[i][j] = 0.0;
          tauSGET[i][j] = 0.0;
      ...
      const DblType CM43 =
        ams_utils::get_M43_constant<DblType, kynema-ugf_ngp::NDimMax>(D, CMdeg);
      ...
      for (int i = 0; i < kynema-ugf_ngp::NDimMax; ++i) {
        for (int j = 0; j < kynema-ugf_ngp::NDimMax; ++j) {
          ...
          tauSGRS[i][j] = avgDudx.get(mi, i * kynema-ugf_ngp::NDimMax + j) +
                          avgDudx.get(mi, j * kynema-ugf_ngp::NDimMax + i);
          ...
          for (int l = 0; l < kynema-ugf_ngp::NDimMax; ++l) {
            ...
            const DblType fluctDudx_jl =
              dudx.get(mi, j * kynema-ugf_ngp::NDimMax + l) -
              avgDudx.get(mi, j * kynema-ugf_ngp::NDimMax + l);
            const DblType fluctDudx_il =
              dudx.get(mi, i * kynema-ugf_ngp::NDimMax + l) -
              avgDudx.get(mi, i * kynema-ugf_ngp::NDimMax + l);
            tauSGET[i][j] +=
              coeffSGET * arScale *
              (M43[i][l] * fluctDudx_jl + M43[j][l] * fluctDudx_il);
          }
          tauSGET[i][j] += arInvScale * tvisc.get(mi, 0) / density.get(mi, 0) *
                           (dudx.get(mi, i * kynema-ugf_ngp::NDimMax + j) -
                            avgDudx.get(mi, i * kynema-ugf_ngp::NDimMax + j) +
                            dudx.get(mi, j * kynema-ugf_ngp::NDimMax + i) -
                            avgDudx.get(mi, j * kynema-ugf_ngp::NDimMax + i));
        }
      }
      ...
      for (int i = 0; i < kynema-ugf_ngp::NDimMax; ++i)
        tauSGET[i][i] -= tauSGET_tr / kynema-ugf_ngp::NDimMax;
      ...
      for (int i = 0; i < kynema-ugf_ngp::NDimMax; ++i) {
        for (int j = 0; j < kynema-ugf_ngp::NDimMax; ++j) {
          for (int l = 0; l < kynema-ugf_ngp::NDimMax; ++l) {
            Psgs[i][j] += tau[i][l] * dudx.get(mi, l * kynema-ugf_ngp::NDimMax + j) +
                          tau[j][l] * dudx.get(mi, l * kynema-ugf_ngp::NDimMax + i);
          }
          Psgs[i][j] *= 0.5;
        }
      }
