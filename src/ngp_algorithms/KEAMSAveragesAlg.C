...
KEAMSAveragesAlg::KEAMSAveragesAlg(Realm& realm, stk::mesh::Part* part)
  : AMSAveragesAlg(realm, part),
    ...
    totDissipationRate_(
      get_field_ordinal(realm.meta_data(), "total_dissipation_rate")),
...
void
KEAMSAveragesAlg::execute()
{
  using Traits = nalu_ngp::NGPMeshTraits<stk::mesh::NgpMesh>;

  const auto& meta = realm_.meta_data();
  if (meta.spatial_dimension() != 3) {
    throw std::runtime_error("KEAMSAveragesAlg only supported in 3D.");
  }
  ...
  const auto tdr = fieldMgr.get_field<double>(totDissipationRate_);
  ...
  nalu_ngp::run_entity_algorithm(
    "KEAMSAveragesAlg_computeAverages", ngpMesh, stk::topology::NODE_RANK, sel,
    KOKKOS_LAMBDA(const Traits::MeshIndex& mi) {
      ...
      avgTime.get(mi, 0) = tke.get(mi, 0) / tdr.get(mi, 0);
      ...
      const DblType epsilon13 = stk::math::pow(tdr.get(mi, 0), 1.0 / 3.0);
