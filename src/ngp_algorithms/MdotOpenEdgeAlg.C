...
MdotOpenEdgeAlg<BcAlgTraits>::MdotOpenEdgeAlg(
  ...
  : Algorithm(realm, part),
    ...
    coordinates_(get_field_ordinal(
                   realm.meta_data(), realm.get_coordinates_name())),
...
void MdotOpenEdgeAlg<BcAlgTraits>::execute()
{
  ...
  const auto mdotOps = nalu_ngp::simd_face_elem_field_updater(ngpMesh, openMdot);
  ...
  const DoubleType pstabFac = realm_.solutionOptions_->activateOpenMdotCorrection_
    ? 0.0 : 1.0;
