...
MomentumKEAMSDiffEdgeKernel::MomentumKEAMSDiffEdgeKernel(
  const stk::mesh::BulkData& bulk, const SolutionOptions& solnOpts)
  : NGPEdgeKernel<MomentumKEAMSDiffEdgeKernel>(),
...
void
MomentumKEAMSDiffEdgeKernel::setup(Realm& realm)
{
...
void
MomentumKEAMSDiffEdgeKernel::execute(
  ...
  const stk::mesh::FastMeshIndex& nodeR)
{
  ...
  const EdgeKernelTraits::DblType tdrIp =
    0.5 * (stk::math::max(tdr_.get(nodeL, 0), 1.0e-12) +
