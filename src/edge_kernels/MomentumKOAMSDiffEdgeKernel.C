...
MomentumKOAMSDiffEdgeKernel::MomentumKOAMSDiffEdgeKernel(
  const stk::mesh::BulkData& bulk, const SolutionOptions& solnOpts)
  : NGPEdgeKernel<MomentumKOAMSDiffEdgeKernel>(),
...
void
MomentumKOAMSDiffEdgeKernel::setup(Realm& realm)
{
...
KOKKOS_FUNCTION
void
MomentumKOAMSDiffEdgeKernel::execute(
  ...
  const stk::mesh::FastMeshIndex& nodeR)
{
  ...
  const EdgeKernelTraits::DblType epsilon13Ip =
    stk::math::pow(betaStarLowRe * tkeIp * sdrIp, 1.0 / 3.0);
