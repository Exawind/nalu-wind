...
void TpetraLinearSystem::compute_graph_row_lengths(const std::vector<stk::mesh::Entity>& rowEntities,
                                                   ...
                                                   stk::CommNeighbors& commNeighbors)
{
  LinSys::HostRowLengths hostSharedNotOwnedRowLengths = sharedNotOwnedRowLengths.view<HostSpace>();
  LinSys::HostRowLengths hostLocallyOwnedRowLengths = locallyOwnedRowLengths.view<HostSpace>();
  ...
  for(size_t i=0; i<rowEntities.size(); ++i)
  {
    ...
    add_to_length(hostLocallyOwnedRowLengths, hostSharedNotOwnedRowLengths, numDof_, lid_a, maxOwnedRowId_,
    ...
    for(size_t ii=0; ii<numColEntities; ++ii) {
        ...
        add_to_length(hostLocallyOwnedRowLengths, hostSharedNotOwnedRowLengths, numDof_, lid_b, maxOwnedRowId_, entity_b_owned, 1);
                                                  ...
                                                  LocalGraphArrays& sharedNotOwnedGraph)
{
  ...
  for(size_t i=0; i<rowEntities.size(); ++i) {
    ...
    ThrowRequireMsg(entityToColLIDHost_[entity_a.local_offset()] != -1 ,  "insert_graph_connections bad lid ");
    ...
    for(size_t j=0; j<numColEntities; ++j) {
      ...
      ThrowRequireMsg(entityToColLIDHost_[entity_b.local_offset()] != -1 , "insert_graph_connections bad lid #2 ");
...

    {
      ...
      insert_single_dof_row_into_graph(crsGraph, entityToLIDHost_[entity_a.local_offset()], maxOwnedRowId_, numDof_, numColEntities, localDofs_b);
    }
    ...
    for(unsigned j=0; j<numColEntities; ++j) {
      if (entities_b[j] != entity_a) {
        ...
        insert_single_dof_row_into_graph(crsGraph, entityToLIDHost_[entities_b[j].local_offset()], maxOwnedRowId_, numDof_, 1, localDofs_a);
      }
    }
  }
}
    ...
    for(size_t i=0; i<b.size(); ++i) {
      ...
      if (iter != myLIDs_.end()) {
        ...
        if (nodeIds[i] != bulk.identifier(node)) {
          ...
          if (master != node) {
            entityToLIDHost_[master.local_offset()] = entityToLIDHost_[node.local_offset()];
          }
        }
      }
    }
        ...
        for(size_t i=0; i<b.size(); ++i) {
            ...
            if (nodeIds[i] != bulk.identifier(node)) {
              ...
              if (bulk.is_valid(master) && master != node) {
                ...
                entityToColLIDHost_[node.local_offset()] = totalColsMap_->getLocalElement(gid);
              }
...
void TpetraLinearSystem::finalizeLinearSystem()
{
  ...
  LinSys::HostRowLengths globalRowLengths = sharedNotOwnedRowLengths.view<HostSpace>();
  LinSys::HostRowLengths ownedRowLengths = locallyOwnedRowLengths.view<HostSpace>();
...
void TpetraLinearSystem::free_coeff_applier(CoeffApplier* coeffApplier)
{
                                 ...
                                 const char *  /* trace_tag */)
{
  ...
  sum_into(
      ...
      rhs, lhs,
      localIds, sortPermutation,
      entityToLIDHost_, entityToColLIDHost_,
      maxOwnedRowId_, maxSharedNotOwnedRowId_,
      numDof_);
}
    ...
    const double rhs_residual)
{
  reset_rows(getOwnedLocalMatrix(), getSharedNotOwnedLocalMatrix(),
             getOwnedLocalRhs(), getSharedNotOwnedLocalRhs(),
             numNodes, nodeList, beginPos, endPos, diag_value, rhs_residual,
             entityToLIDHost_, maxOwnedRowId_, maxSharedNotOwnedRowId_);
}
