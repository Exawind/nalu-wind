...
void Actuator::load( const YAML::Node &y_node){
  ...
  if (y_actuator) {
    ...
    get_if_present(y_actuator, "search_method", searchMethodName, searchMethodName);
...
Actuator::manage_ghosting()
{
  ...
  if (g_needToGhostCount > 0) {
    NaluEnv::self().naluOutputP0() << get_class_name() + " alg will ghost a number of entities: "
    ...
    NaluEnv::self().naluOutputP0() << get_class_name() + " alg will NOT ghost entities: " << std::endl;
  }
}
  ...
  for( ii=searchKeyPair_.begin(); ii!=searchKeyPair_.end(); ++ii ) {
    ...
    if ( theRank == pt_proc ) {
      ...
      const double nearestDistance = meSCS->isInElement(&elementCoords[0],
                                                        &(actuatorPointInfo->centroidCoords_[0]),
                                                        &(isoParCoords[0]));
