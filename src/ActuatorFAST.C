  ...
  const YAML::Node & y_node)
{
  ...
  if (y_actuator) {
	...
	if (fi.tStart == 0) {
	  ...
	  throw std::runtime_error(get_class_name()+": simStart type not consistent with start time for FAST");
	}
      ...
      get_required(y_actuator, "t_max", fi.tMax); // tMax is the total duration to which you want to run FAST. This should be the same or greater than the max time given in the FAST fst file. Choose this carefully as FAST writes the output file only at this point if you choose the binary file output.
	...
	if (y_actuator["Turbine" + std::to_string(iTurb)]) {
	  ...
	  readTurbineData(iTurb, fi, y_actuator["Turbine" + std::to_string(iTurb)] );
	} else {
...
void ActuatorFAST::readTurbineData(int iTurb, fast::fastInputs & fi, YAML::Node turbNode) {
...
ActuatorFAST::setup()
{
  ...
  if (std::abs(dtNalu - tStepRatio_ * fi.dtFAST) < 0.001) {// TODO: Fix arbitrary number 0.001
    ...
    throw std::runtime_error("ActuatorFAST: Ratio of Nalu's time step is not an integral multiple of FAST time step");
  }
...
ActuatorFAST::populate_candidate_procs()
{
  ...
  for ( size_t k = 0; k < searchTargetNames_.size(); ++k ) {
    ...
    if ( NULL != thePart )
      ...
      throw std::runtime_error("ActuatorFAST: Part is null" + searchTargetNames_[k]);
  }
...
ActuatorFAST::create_actuator_point_info_map() {
  ...
  for ( size_t iTurb = 0; iTurb < actuatorInfo_.size(); ++iTurb ) {
    ...
    const auto actuatorInfo = dynamic_cast<ActuatorFASTInfo*>(actuatorInfo_[iTurb].get());
