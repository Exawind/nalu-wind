...
void
InputOutputRealm::initialize_epilog()
{
}
...
InputOutputRealm::register_io_fields()
{
  ...
  for (size_t k = 0; k < inputOutputFieldInfo_.size(); ++k) {
    ...
    std::vector<std::string> targetNames =
      inputOutputFieldInfo_[k]->targetNames_;

    // sanity check on the type
    if (fieldType != "node_rank") {
      throw std::runtime_error(
        "Input/output realm only supports nodal_rank types");
    }
    ...
    for (size_t j = 0; j < targetNames.size(); ++j) {
      ...
      if (NULL == targetPart) {
        throw std::runtime_error(
          "Sorry, no part name found by the name: " + targetName +
          " for field: " + fieldName);
      } else {
...
InputOutputRealm::load(const YAML::Node& node)
{
  ...
  if (y_field) {
    ...
    const YAML::Node y_specs =
      expect_sequence(y_field, "specifications", false);
    if (y_specs) {
