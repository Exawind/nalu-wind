...
AssembleMomentumEdgeABLTopBC::execute()
{
  ...
  std::vector<double> wSamp(imax_ * jmax_), uBC(imax_ * jmax_),
    vBC(imax_ * jmax_), wBC(imax_ * jmax_), work(imax_ * jmax_), UAvg(9, 0.0);
  int i, j, ii;
  int nx = imax_ - 1;
  int ny = jmax_ - 1;
  ...
  MPI_Allgatherv(
    wSamp.data(), nSamp, MPI_DOUBLE, work.data(), sampleDistrib_.data(),
    displ_.data(), MPI_DOUBLE, bulk_data.parallel());
  ...
  MPI_Allreduce(
    MPI_IN_PLACE, UAvg.data(), 9, MPI_DOUBLE, MPI_SUM, bulk_data.parallel());
...
AssembleMomentumEdgeABLTopBC::initialize()
{
  ...
  int i, ii, ix, ixInflow, iy, iyInflow, iz, izSample, imaxjmax, j, n, nx, ny,
    nz, iOff, count, count1, countXInflow, countYInflow, nSamp;
  ...
  if (
    std::abs(horizBC_[0]) > 1 || std::abs(horizBC_[1]) > 1 ||
  ...
  if (horizBC_[0] == 0 && horizBC_[2] == 0)
    horizBCType_ = 0; // periodic-periodic
  if (std::abs(horizBC_[0]) == 1 && horizBC_[2] == 0)
    horizBCType_ = 1; // inflow  -periodic
  if (horizBC_[0] == 0 && std::abs(horizBC_[2]) == 1)
    horizBCType_ = 2; // periodic-inflow
  if (std::abs(horizBC_[0]) == 1 && std::abs(horizBC_[2]) == 1)
    horizBCType_ = 3; // inflow  -inflow
  ...
  switch (horizBCType_) {
  case 0:
    planFourier2dF_ = fftw_plan_dft_r2c_2d(
      ny, nx, work.data(), reinterpret_cast<fftw_complex*>(workC.data()),
      flags);
    planFourier2dB_ = fftw_plan_dft_c2r_2d(
      ny, nx, reinterpret_cast<fftw_complex*>(workC.data()), work.data(),
      flags);
    break;
  case 1:
    planSinx_ =
    ...
    planFourieryF_ = fftw_plan_dft_r2c_1d(
      ny, work.data(), reinterpret_cast<fftw_complex*>(workC.data()), flags);
    planFourieryB_ = fftw_plan_dft_c2r_1d(
      ny, reinterpret_cast<fftw_complex*>(workC.data()), work.data(), flags);
    break;
  case 3:
    planSinx_ =
      fftw_plan_r2r_1d(nx - 1, work.data(), work.data(), FFTW_RODFT00, flags);
  ...
  MPI_Allreduce(
    MPI_IN_PLACE, zGrid.data(), kmax_, MPI_DOUBLE, MPI_MAX,
    bulk_data.parallel());
  ...
  for (iz = 0; iz < kmax_; ++iz) {
    if (zGrid[iz] <= zSample_ && zGrid[iz + 1] > zSample_) {
      break;
    }
  }
  if ((zSample_ - zGrid[iz]) > 0.5 * (zGrid[iz + 1] - zGrid[iz])) {
    iz++;
  }
  izSample = iz;
  ...
  MPI_Allreduce(
    MPI_IN_PLACE, xMin.data(), 2, MPI_DOUBLE, MPI_MIN, bulk_data.parallel());
  MPI_Allreduce(
    MPI_IN_PLACE, xMax.data(), 2, MPI_DOUBLE, MPI_MAX, bulk_data.parallel());
  ...
  for (iy = 0; iy < ny; ++iy) {
    for (ix = 0; ix < nx; ++ix) {
      ...
      if (bulk_data.is_valid(nodeSamp) && bulk_data.bucket(nodeSamp).owned()) {
  ...
  for (iy = 0; iy < jmax_; ++iy) {
    for (ix = 0; ix < imax_; ++ix) {
      ...
      if (bulk_data.is_valid(nodeBC) && bulk_data.bucket(nodeBC).owned()) {
  ...
  MPI_Allgather(
    &nXInflow_, 1, MPI_INT, sampleDistrib_.data(), 1, MPI_INT,
    bulk_data.parallel());
  ...
  MPI_Allgatherv(
    indexMapXInflow.data(), nXInflow_, MPI_INT, indexMapSampGlobal_.data(),
    sampleDistrib_.data(), displ_.data(), MPI_INT, bulk_data.parallel());
  ...
  MPI_Allgather(
    &nYInflow_, 1, MPI_INT, sampleDistrib_.data(), 1, MPI_INT,
    bulk_data.parallel());
  ...
  MPI_Allgatherv(
    indexMapYInflow.data(), nYInflow_, MPI_INT, indexMapSampGlobal_.data(),
    sampleDistrib_.data(), displ_.data(), MPI_INT, bulk_data.parallel());
  ...
  MPI_Allgather(
    &nSamp, 1, MPI_INT, sampleDistrib_.data(), 1, MPI_INT,
    bulk_data.parallel());
  ...
  MPI_Allgatherv(
    indexMapSamp.data(), nSamp, MPI_INT, indexMapSampGlobal_.data(),
    sampleDistrib_.data(), displ_.data(), MPI_INT, bulk_data.parallel());
  ...
  for (n = 0; n < nprocs; ++n) {
    for (i = displ_[n]; i < displ_[n + 1]; i++) {
      for (j = displ_[n + 1]; j < displ_[nprocs]; ++j) {
        if (indexMapSampGlobal_[i] == indexMapSampGlobal_[j]) {
          if (
            indexMapSampGlobal_[i] < nx || (indexMapSampGlobal_[i] % nx) == 0) {
            indexMapSampGlobal_[j] = -1;
          } else {
            indexMapSampGlobal_[i] = -1;
          }
  ...
  std::vector<double>& wBC)
{
  ...
  fftw_execute_dft_c2r(
    planFourier2dB_, reinterpret_cast<fftw_complex*>(uCoef.data()), uBC.data());
  fftw_execute_dft_c2r(
    planFourier2dB_, reinterpret_cast<fftw_complex*>(vCoef.data()), vBC.data());
  fftw_execute_dft_c2r(
    planFourier2dB_, reinterpret_cast<fftw_complex*>(wCoef.data()), wBC.data());
  ...
  std::vector<double>& wBC)
{
  ...
  for (i = 0; i < nx; ++i) {
    ...
    j1 = i * (ny / 2 + 1);
    fftw_execute_dft_r2c(
      planFourieryF_, &work[j0], reinterpret_cast<fftw_complex*>(&wCoef[j1]));
  }
  ...
  for (i = 0; i < nx; ++i) {
    ...
    j1 = i * (ny / 2 + 1);
    fftw_execute_dft_c2r(
      planFourieryB_, reinterpret_cast<fftw_complex*>(&uCoef[j1]), &work[j0]);
  }
  for (j = 0; j < ny; ++j) {
    i0 = j * imax_;
    for (i = 0; i < nx; ++i) {
  ...
  for (i = 0; i < nx; ++i) {
    ...
    j1 = i * (ny / 2 + 1);
    fftw_execute_dft_c2r(
      planFourieryB_, reinterpret_cast<fftw_complex*>(&vCoef[j1]), &work[j0]);
  }
  for (j = 0; j < ny; ++j) {
    i0 = j * imax_;
    for (i = 0; i < nx; ++i) {
  ...
  for (i = 0; i < nx; ++i) {
    ...
    j1 = i * (ny / 2 + 1);
    fftw_execute_dft_c2r(
      planFourieryB_, reinterpret_cast<fftw_complex*>(&wCoef[j1]), &work[j0]);
  }
  for (j = 0; j < ny; ++j) {
    i0 = j * imax_;
    for (i = 0; i < nx; ++i) {
