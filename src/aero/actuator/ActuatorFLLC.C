...
#include <aero/actuator/ActuatorScalingFLLC.h>
...
FilteredLiftingLineCorrection::compute_lift_force_distribution()
{
  ...
  for (auto&& info : bladeDistInfo_) {
    ...
    Kokkos::parallel_for("extract lift", range_policy, [=, *this](int i) {
...
FilteredLiftingLineCorrection::grad_lift_force_distribution()
{
  ...
  for (auto&& info : bladeDistInfo_) {
    ...
    Kokkos::parallel_for("compute dG", range_policy, [=, *this](int i) {
...
FilteredLiftingLineCorrection::compute_induced_velocities()
{
  ...
  for (auto&& info : bladeDistInfo_) {
    ...
    Kokkos::parallel_for("compute flucs", range_policy, [=, *this](int index) {
