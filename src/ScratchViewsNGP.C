...
int get_num_scalars_pre_req_data(ElemDataRequestsGPU& dataNeeded, int nDim)
{
...
int get_num_scalars_pre_req_data(ElemDataRequestsGPU& dataNeeded, int nDim, const ScratchMeInfo &meInfo)
{
  ...
  bool fillMEViews)
{
  ...
  for(unsigned f=0; f<neededFields.size(); ++f) {
    ...
    if (fieldEntityRank==stk::topology::EDGE_RANK || fieldEntityRank==stk::topology::FACE_RANK || fieldEntityRank==stk::topology::ELEM_RANK) {
      if (isTensorField) {
        auto& shmemView = prereqData.get_scratch_view_2D(fieldInfo.field.get_ordinal());
        gather_elem_tensor_field(fieldInfo.field, entityIndex, scalarsDim1, fieldInfo.scalarsDim2, shmemView);
      }
      else {
        auto& shmemView = prereqData.get_scratch_view_1D(fieldInfo.field.get_ordinal());
      ...
      if (isTensorField) {
        auto& shmemView3D = prereqData.get_scratch_view_3D(fieldInfo.field.get_ordinal());
        ...
        if (scalarsDim1 == 1) {
          auto& shmemView1D = prereqData.get_scratch_view_1D(fieldInfo.field.get_ordinal());
          gather_elem_node_field(fieldInfo.field, ngpMesh, prereqData.elemNodes, shmemView1D);
        }
        else {
          auto& shmemView2D = prereqData.get_scratch_view_2D(fieldInfo.field.get_ordinal());
          if (scalarsDim1 == 3) {
