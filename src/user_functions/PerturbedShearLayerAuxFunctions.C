...
double
wrap_value(double value, double wrap)
{
...
public:
  double funu(double y)
  {
    ...
    if (do_wrap) {
      return (
        std::tanh(inv_initial_vorticity_thickness * (ymod - 2 * M_PI)) -
  ...
  double funvw(double y)
  {
    ...
    return perturb_mag * std::exp(
                           -100 / (size_ratio_x * size_ratio_x) *
                           (ymod - M_PI) * (ymod - M_PI));
  }
  ...
  const unsigned /*endPos*/) const
{
  ...
  for (unsigned p = 0; p < numPoints; ++p) {
    ...
    fieldPtr[3 * p + 0] =
      slh.funu(y) +
      slh.funvw(y) * (std::sin(2 * kx * x) + 0.01 * std::cos(kz * z) + r1(rng));
    fieldPtr[3 * p + 1] =
      slh.funvw(y) * (std::sin(kx * x) + 0.01 * std::cos(2 * kz * z) + r1(rng));
    fieldPtr[3 * p + 2] =
      slh.funvw(y) * (std::cos(2 * kx * x) + 0.01 * std::sin(kz * z) + r1(rng));
  }
}

PerturbedShearLayerMixFracAuxFunction::PerturbedShearLayerMixFracAuxFunction()
  : AuxFunction(0, 1)
{
}
