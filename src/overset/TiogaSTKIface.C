...
TiogaSTKIface::TiogaSTKIface(
  sierra::kynema-ugf::OversetManagerTIOGA& oversetManager,
...
TiogaSTKIface::post_connectivity_work(const bool isDecoupled)
{
  ...
  sierra::kynema-ugf::ScalarIntFieldType* ibf =
...
TiogaSTKIface::update_ghosting()
{
  ...
  if ((global[0] > 0) || (global[1] > 0)) {
    ...
    sierra::kynema-ugf::populate_ghost_comm_procs(
...
TiogaSTKIface::populate_overset_info()
{
  ...
  for (size_t i = 0; i < numReceptors; i++) {
    ...
    sierra::kynema-ugf::OversetInfo* oinfo =
      new sierra::kynema-ugf::OversetInfo(node, nDim);
    ...
    if (nearestDistance > (1.0 + 1.0e-8))
      sierra::kynema-ugf::KynemaUGFEnv::self().kynema-ugfOutput()
...
TiogaSTKIface::overset_update_fields(
  const std::vector<sierra::kynema-ugf::OversetFieldData>& fields)
{
...
{
  constexpr int row_major = 0;
  sierra::kynema-ugf::OversetFieldData fdata{field, nrows, ncols};
...
TiogaSTKIface::post_connectivity_sync()
{
  ...
  ngpFringes =
    sierra::kynema-ugf::OversetManager::EntityList("ngp_fringe_list", fringes.size());
  ngpHoles =
    sierra::kynema-ugf::OversetManager::EntityList("ngp_hole_list", holes.size());
