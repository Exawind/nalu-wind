...
SpecificDissipationRateSSTDESSrcElemKernel<AlgTraits>::
    ...
    cDESkw_(solnOpts.get_turb_model_constant(TM_cDESkw))
{
  ...
  cellLengthScale_ = get_field_ordinal(metaData, "sst_max_length_scale");
  ...
  dataPreReqs.add_gathered_nodal_field(cellLengthScale_, 1);
...
template <typename AlgTraits>
void
SpecificDissipationRateSSTDESSrcElemKernel<AlgTraits>::execute(
  ...
  ScratchViews<DoubleType, DeviceTeamHandleType, DeviceShmem>& scratchViews)
{
  ...
  const auto& v_cellLengthScale = scratchViews.get_scratch_view_1D(cellLengthScale_);
  ...
  for (int ip = 0; ip < AlgTraits::numScvIp_; ++ip) {
    ...
    DoubleType cellLengthScale = 0.0;
    ...
    for (int ic = 0; ic < AlgTraits::nodesPerElement_; ++ic) {
      ...
      cellLengthScale += r * v_cellLengthScale(ic);
    ...
    const DoubleType cDES  = om_fOneBlend * cDESke_ + fOneBlend * cDESkw_;

    // limit the production consistently
    const DoubleType small = 1.0e-16;
    const DoubleType eddyLengthRANS = stk::math::sqrt(tke) / stk::math::max(betaStar_ * sdr, small);
    const DoubleType eddyLengthLES = cDES * cellLengthScale;
    const DoubleType eddyLengthDES = stk::math::min(eddyLengthRANS, eddyLengthLES);

    const DoubleType Dk = rho * stk::math::sqrt(tke) * tke / stk::math::max(eddyLengthDES, small);
...
INSTANTIATE_KERNEL(SpecificDissipationRateSSTDESSrcElemKernel)
