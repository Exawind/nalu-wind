...
//--------------------------------------------------------------------------
std::vector<int> HexNElementDescription::edge_node_ordinals()
{
...
void HexNElementDescription::set_edge_node_connectivities()
{
  ...
  for (int edgeOrdinal = 0; edgeOrdinal < numEdges; ++edgeOrdinal) {
    ...
    edgeNodeConnectivities[edgeMap[edgeOrdinal]] = std::vector<int>{beginIterator, endIterator};
    beginIterator = endIterator;
  }
}
//--------------------------------------------------------------------------
std::vector<int> HexNElementDescription::face_node_ordinals()
{
  ...
  const int firstfaceNodeNumber = nodesInBaseElement + numEdges * newNodesPerEdge;
...
void HexNElementDescription::set_face_node_connectivities()
{
  ...
  for (int faceOrdinal = 0; faceOrdinal < numFaces; ++faceOrdinal) {
    ...
    faceNodeConnectivities[faceMap[faceOrdinal]] = std::vector<int>{beginIterator, endIterator};
    beginIterator = endIterator;
  }
}
//--------------------------------------------------------------------------
std::vector<int> HexNElementDescription::volume_node_ordinals()
{
  ...
  int firstVolumeNodeNumber = nodesInBaseElement + numEdges * newNodesPerEdge + numFaces * newNodesPerFace;
...
std::array<std::vector<int>, 4> edge_node_connectivities(int p, int start_index)
{
  ...
  for (int edgeOrdinal = 0; edgeOrdinal < numEdges; ++edgeOrdinal) {
    auto endIterator = beginIterator + newNodesPerEdge;
    edgeNodeConnectivities[edgeOrdinal] = std::vector<int>{beginIterator,endIterator};
    beginIterator = endIterator;
  }
  return edgeNodeConnectivities;
}

std::vector<int> volume_node_connectivities(int p, int start_index)
{
...
int tensor_edge_index(int p, int i, int j, int edge_ordinal)
{
  ...
  switch (edge_ordinal) {
    case 0: return i + 1; // bottom, left-to-right
    case 1: return p + nodes1D * (j + 1); // right-side, bottom-to-top.
    case 2: return p - (i + 1) + nodes1D *  p; // top, right-to-left
    case 3: return nodes1D * (p - (j + 1)); // left-side, top-to-bottom
    default: return -1;
  }
}

int stk_edge_index(int i, int j, int edge_ordinal)
{
  return (edge_ordinal == 0 || edge_ordinal == 2) ? i : j;
}

std::vector<int> node_map_bc(int p)
{
  ...
  const auto newVolumeNodes =  volume_node_connectivities(p, numVertices + edgeNodeConnectivities.size() *(p-1));
  for (int j = 0; j < p-1; ++j) {
    for (int i = 0; i < p-1; ++i) {
...
void HexNElementDescription::set_tensor_product_node_mappings()
{
  ...
  if (polyOrder > 1) {
    ...
    for (int k = 0; k < newNodesPerEdge; ++k) {
      for (int j = 0; j < newNodesPerEdge; ++j) {
        for (int i = 0; i < newNodesPerEdge; ++i) {
          node_map(i + 1, j + 1, k + 1) = volumeNodeConnectivities.at(i + newNodesPerEdge * (j + newNodesPerEdge * k));
        }
      }
    }
  }
