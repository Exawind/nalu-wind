...
bool
part_vector_is_valid_and_nonempty(const stk::mesh::PartVector& parts)
{
  return (
    std::find(parts.begin(), parts.end(), nullptr) == parts.end() &&
    !parts.empty());
}
//--------------------------------------------------------------------------
bool
check_part_topo(const stk::mesh::Part& part)
{
  ...
  if (topo.rank() == stk::topology::ELEM_RANK) {
    is_valid_elem_rank = (dim == 2) ? topo == stk::topology::QUAD_4_2D
                                    : topo == stk::topology::HEX_8;
  }

  bool is_valid_side_rank = false;
  if (topo.rank() == part.mesh_meta_data().side_rank()) {
    is_valid_side_rank = (dim == 2) ? topo == stk::topology::LINE_2
                                    : topo == stk::topology::QUAD_4;
  }
...
//--------------------------------------------------------------------------
std::string
super_subset_part_name(std::string base_name)
{
  ...
  std::string name =
    first_token + super_element_suffix() + "_super_superside" + last_token;

  return name;
}
std::string
super_subset_part_name(
...
base_element_part_name(std::string super_name)
{
  ...
  return (super_name.erase(
    super_name.find(super_element_suffix()), super_element_suffix().length()));
}
//--------------------------------------------------------------------------
stk::mesh::Part*
...
{
  return (part.mesh_meta_data().get_part(
    super_subset_part_name(part.name(), numElemNodes, numSideNodes)));
}
//--------------------------------------------------------------------------
stk::mesh::Part*
super_subset_part(const stk::mesh::Part& part)
{
...
{
  return (super_elem_part.mesh_meta_data().get_part(
    base_element_part_name(super_elem_part.name())));
}
//--------------------------------------------------------------------------
stk::mesh::Part*
super_elem_part(const stk::mesh::Part* part)
...
//------------------------------------------------------------------------
bool
is_super(const stk::mesh::Part* p)
{
  return (
    p->topology().is_superedge() || p->topology().is_superface() ||
    p->topology().is_superelement());
}
//------------------------------------------------------------------------
bool
is_side(const stk::mesh::Part* p)
{
  return (p->mesh_meta_data().side_rank() == p->topology().side_rank());
}
//------------------------------------------------------------------------
bool
is_super_side(const stk::mesh::Part* p)
{
  ...
  bool with_subsets)
{
  ...
  std::copy_if(
    parts.begin(), parts.end(), std::back_inserter(elemParts),
    [rank](stk::mesh::Part* p) {
...
//--------------------------------------------------------------------------
stk::mesh::PartVector
base_elem_parts(const stk::mesh::PartVector& parts)
{
  return base_ranked_parts(parts, stk::topology::ELEM_RANK, true);
}
//--------------------------------------------------------------------------
stk::mesh::PartVector
base_edge_parts(const stk::mesh::PartVector& parts)
{
  return base_ranked_parts(parts, stk::topology::EDGE_RANK, true);
}
//--------------------------------------------------------------------------
stk::mesh::PartVector
base_face_parts(const stk::mesh::PartVector& parts)
{
...
only_super_elem_parts(const stk::mesh::PartVector& parts)
{
  ...
  std::copy_if(
    parts.begin(), parts.end(), std::back_inserter(elemParts),
    [](stk::mesh::Part* p) { return (p->topology().is_superelement()); });
  return elemParts;
}
//--------------------------------------------------------------------------
stk::mesh::PartVector
only_super_side_parts(const stk::mesh::PartVector& parts)
{
  ...
  std::copy_if(
    parts.begin(), parts.end(), std::back_inserter(faceParts),
    [](stk::mesh::Part* p) {
...
//--------------------------------------------------------------------------
stk::topology
get_promoted_elem_topo(int dim, int order)
{
  return (stk::create_superelement_topology(
    static_cast<unsigned>(std::pow(order + 1, dim))));
}
//--------------------------------------------------------------------------
stk::topology
face_topology_for_order(int order)
{
  if (order == 1) {
