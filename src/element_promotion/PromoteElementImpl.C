...
interpolate_coordinate(
  const std::array<double,24>& baseCoords, const std::array<double,3>& isoParCoords, std::array<double,3>& physCoords)
{
  ...
  const VectorFieldType& coordField)
{
  ...
  bucket_loop(elem_buckets, [&](const stk::mesh::Entity elem) {
    ...
    for (int ord = 8; ord < desc.nodesPerElement; ++ord) {
      ...
      std::array<double, 3> isoParCoords = {{nodeLocs1D[indices[0]], nodeLocs1D[indices[1]], nodeLocs1D[indices[2]]}};
