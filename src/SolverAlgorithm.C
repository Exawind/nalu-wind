...
{
  using ScalarIntFieldType = sierra::kynema-ugf::ScalarIntFieldType;
...
{
  if (extractDiagonal_) {
    diagField_ = kynema-ugf_ngp::get_ngp_field(
      eqSystem->realm_.mesh_info(), eqSystem->get_diagonal_field()->name());
  }

  if (hasOverset_) {
    iblankField_ =
      kynema-ugf_ngp::get_ngp_field<int>(eqSystem->realm_.mesh_info(), "iblank");
  }
}
  ...
  SharedMemView<double**, DeviceShmem>& lhs) const
{
  constexpr bool forceAtomic = std::is_same<
    sierra::kynema-ugf::DeviceSpace, Kokkos::DefaultExecutionSpace>::value;
