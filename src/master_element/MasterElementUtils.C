...
bool
isoparameteric_coordinates_for_point_3d(
  ...
  double deltaLimit)
{
  ...
  do {
    ...
    error_vec[0] = pointCoord[0] -
                   ddot(weights.data(), elemNodalCoords + 0 * nNodes, nNodes);
    error_vec[1] = pointCoord[1] -
                   ddot(weights.data(), elemNodalCoords + 1 * nNodes, nNodes);
    error_vec[2] = pointCoord[2] -
                   ddot(weights.data(), elemNodalCoords + 2 * nNodes, nNodes);
...
bool
isoparameteric_coordinates_for_point_2d(
  sierra::nalu::LagrangeBasis& basis,
  const double* POINTER_RESTRICT elemNodalCoords,
  const double* POINTER_RESTRICT pointCoord,
  double* POINTER_RESTRICT isoParCoord,
