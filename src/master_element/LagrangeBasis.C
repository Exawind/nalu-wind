...
Kokkos::View<double*>
barycentric_weights(const Kokkos::View<double*>& nodelocs)
{
  const auto numNodes = nodelocs.extent(0);
  auto barycentricWeights =
    Kokkos::View<double*>("barycentric_weights", numNodes);
  for (unsigned k = 0; k < numNodes; ++k)
    barycentricWeights[k] = 1.0;
...
void
LagrangeBasis::fill_interpolation_weights(
  const double* isoParCoords, double* weights) const
{
  for (unsigned k = 0; k < numNodes_; ++k) {
    double interpolant_weight = 1.0;
    for (unsigned d = 0; d < dim_; ++d) {
      interpolant_weight *=
        basis1D_.interpolation_weight(isoParCoords[d], indicesMap_(k, d));
    }
...
void
LagrangeBasis::fill_derivative_weights(
  const double* isoParCoords, double* weights) const
{
  for (unsigned k = 0; k < numNodes_; ++k) {
    for (unsigned dj = 0; dj < dim_; ++dj) {
      ...
      for (unsigned di = 0; di < dim_; ++di) {
        ...
        derivative_weight *= (di == dj)
                               ? basis1D_.derivative_weight(x, mapped_k)
                               : basis1D_.interpolation_weight(x, mapped_k);
      }
...
LagrangeBasis::point_interpolation_weights(const double* isoParCoords)
{
...
const Kokkos::View<double**>&
LagrangeBasis::point_derivative_weights(const double* isoParCoords)
{
...
Kokkos::View<double**>
LagrangeBasis::eval_basis_weights(const double* intgLoc, int nInt) const
{
...
Kokkos::View<double**>
LagrangeBasis::eval_basis_weights(const Kokkos::View<double**>& intgLoc) const
{
  return eval_basis_weights(intgLoc.data(), intgLoc.extent_int(0));
}

Kokkos::View<double***>
LagrangeBasis::eval_deriv_weights(const double* intgLoc, int nInt) const
{
  Kokkos::View<double***> derivWeights(
    "derivative_weights", nInt, numNodes_, dim_);
  for (int ip = 0; ip < nInt; ++ip) {
...
Kokkos::View<double***>
LagrangeBasis::eval_deriv_weights(const Kokkos::View<double**>& intgLoc) const
{
