  ...
  const std::string& searchMethodName)
{
  ...
  stk::mesh::Selector masterSelect =
    meta_data.locally_owned_part() & stk::mesh::Selector(*masterMeshPart);
  stk::mesh::Selector slaveSelect =
    meta_data.locally_owned_part() & stk::mesh::Selector(*slaveMeshPart);
...
PeriodicManager::build_constraints()
{
  ...
  for (size_t k = 0; k < periodicSelectorPairs_.size(); ++k) {
    determine_translation(
      periodicSelectorPairs_[k].first, periodicSelectorPairs_[k].second,
...
PeriodicManager::augment_periodic_selector_pairs()
{
  ...
  case 2: {
    ...
    periodicSelectorPairs_.push_back(
      std::make_pair(masterA & masterB, slaveA & slaveB));
  ...
  case 3: {
    ...
    periodicSelectorPairs_.push_back(
      std::make_pair(masterA & masterB, slaveA & slaveB));
    periodicSelectorPairs_.push_back(
      std::make_pair(masterB & masterC, slaveB & slaveC));
    periodicSelectorPairs_.push_back(
      std::make_pair(masterA & masterC, slaveA & slaveC));
    periodicSelectorPairs_.push_back(
      std::make_pair(masterA & masterB & masterC, slaveA & slaveB & slaveC));
  ...
  std::vector<double>& rotationVector)
{
  ...
  std::vector<double> local_sum_coords_master(nDim, 0.0),
    global_sum_coords_master(nDim, 0.0);
  size_t numberMasterNodes = 0, g_numberMasterNodes = 0;

  stk::mesh::BucketVector const& master_node_buckets =
    realm_.get_buckets(stk::topology::NODE_RANK, masterSelector);
  ...
  std::vector<double> local_sum_coords_slave(nDim, 0.0),
    global_sum_coords_slave(nDim, 0.0);
  ...
  for (int j = 0; j < nDim; ++j) {
    translationVector[j] =
      (global_sum_coords_master[j] - global_sum_coords_slave[j]) /
      g_numberMasterNodes;
  ...
  const stk::search::SearchMethod searchMethod)
{
  ...
  searchKeyVector_.insert(
    searchKeyVector_.end(), searchKeyPair.begin(), searchKeyPair.end());
}
...
PeriodicManager::error_check()
{
  ...
  stk::mesh::Selector s_locally_owned =
    meta_data.locally_owned_part() & stk::mesh::selectUnion(slavePartVector_);
  stk::mesh::BucketVector const& slave_node_buckets =
...
//--------------------------------------------------------------------------
void
add_range_nodes_to_sharers_of_domain_nodes(
      ...
      if ((theRank == domainProc) && bulk_data.bucket(domainNode).shared()) {
        ...
        if (theRank == rangeProc) {
          stk::mesh::Entity rangeNode =
            bulk_data.get_entity(stk::topology::NODE_RANK, rangeId);
...
PeriodicManager::manage_ghosting_object()
{
  ...
  add_range_nodes_to_sharers_of_domain_nodes(
    bulk_data, searchKeyVector_, sendNodes);
  ...
  deviceMasterSlaves_ =
    KokkosEntityPairView("deviceMasterSlaves", masterSlaveCommunicator_.size());
  hostMasterSlaves_ = Kokkos::create_mirror_view(deviceMasterSlaves_);
  for (size_t i = 0; i < masterSlaveCommunicator_.size(); ++i) {
    const EntityPair& entityPair = masterSlaveCommunicator_[i];
    hostMasterSlaves_(i) = KokkosEntityPair(entityPair);
  }

  Kokkos::deep_copy(deviceMasterSlaves_, hostMasterSlaves_);
}
  ...
  stk::mesh::FieldBase* theField) const
{
  if (NULL != periodicGhosting_) {
    ...
    if (theField->type_is<double>()) {
      std::vector<NGPDoubleFieldType*> fieldVec(
        1, &fieldMgr.get_field<double>(fieldOrd));
      stk::mesh::communicate_field_data(*periodicGhosting_, fieldVec);
    } else if (theField->type_is<stk::mesh::EntityId>()) {
      std::vector<NGPGlobalIdFieldType*> fieldVec(
        1, &fieldMgr.get_field<stk::mesh::EntityId>(fieldOrd));
      stk::mesh::communicate_field_data(*periodicGhosting_, fieldVec);
    } else if (theField->type_is<int>()) {
      std::vector<NGPScalarIntFieldType*> fieldVec(
        1, &fieldMgr.get_field<int>(fieldOrd));
      stk::mesh::communicate_field_data(*periodicGhosting_, fieldVec);
    } else if (theField->type_is<LinSys::GlobalOrdinal>()) {
      std::vector<stk::mesh::NgpField<LinSys::GlobalOrdinal>*> fieldVec(
...
//--------------------------------------------------------------------------
void
PeriodicManager::parallel_communicate_field(stk::mesh::FieldBase* theField)
{
  stk::mesh::BulkData& bulk_data = realm_.bulk_data();
    ...
    if (theField->type_is<double>()) {
      ...
      STK_ThrowRequireMsg(
        false, "Error, field "
                 ...
                 << ") in PeriodicManager::parallel_communicate_field.");
    }
  }
}
  ...
  const bool& doCommunication) const
{
  ...
  if (addSlaves)
    ngp_add_slave_to_master(
      theField, sizeOfField, bypassFieldCheck, doCommunication);
  if (setSlaves)
...
PeriodicManager::apply_max_field(
  stk::mesh::FieldBase* theField, const unsigned& sizeOfField)
{
  ...
  const bool& bypassFieldCheck)
{
  ...
  if (bypassFieldCheck) {
    ...
    for (size_t k = 0; k < masterSlaveCommunicator_.size(); ++k) {
      ...
      // pointer to data
      double* masterField =
        (double*)stk::mesh::field_data(*theField, masterNode);
      const double* slaveField =
        (double*)stk::mesh::field_data(*theField, slaveNode);
      // add in contribution
      for (unsigned j = 0; j < sizeOfField; ++j) {
  ...
  const bool& doCommunication) const
{
  ...
  NGPDoubleFieldType ngpField = realm_.ngp_field_manager().get_field<double>(
    theField->mesh_meta_data_ordinal());
  ...
  if (bypassFieldCheck) {
    ...
    Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const int i) {
        ...
        const stk::mesh::Entity masterNode = entPair.first;
        const stk::mesh::Entity slaveNode = entPair.second;

        const stk::mesh::FastMeshIndex master =
          ngpMesh.fast_mesh_index(masterNode);
        const stk::mesh::FastMeshIndex slave =
          ngpMesh.fast_mesh_index(slaveNode);

        // add in contribution
        for (unsigned j = 0; j < fieldSize; ++j) {
          ngpField.get(master, j) += ngpField.get(slave, j);
        }
      });
  } else {
    // more costly check to see if fields are defined on master/slave nodes
    Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const int i) {
        ...
        if (ngpField.get_num_components_per_entity(master) == fieldSize) {
          // add in contribution
          for (unsigned j = 0; j < fieldSize; ++j) {
            ngpField.get(master, j) += ngpField.get(slave, j);
          }
    ...
    Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const int i) {
        ...
        const stk::mesh::FastMeshIndex slave =
          ngpMesh.fast_mesh_index(slaveNode);
        for (unsigned j = 0; j < fieldSize; ++j) {
          ngpField.get(slave, j) = ngpField.get(master, j);
        }
      });
  } else {
    // more costly check to see if fields are defined on master/slave nodes
